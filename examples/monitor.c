...
static const gchar *get_role_string(MateMixerStreamControlRole role) {
  switch (role) {
...
static const gchar *get_media_role_string(
    MateMixerStreamControlMediaRole role) {
  switch (role) {
...
static const gchar *get_device_switch_role_string(
...
static const gchar *get_stream_switch_role_string(
...
static const gchar *get_direction_string(MateMixerDirection direction) {
  switch (direction) {
...
static gdouble get_volume_percentage(MateMixerStreamControl *control) {
  guint volume;
  guint volume_min;
  guint volume_max;
...
static void print_devices(void) {
  ...
  while (devices != NULL) {
    ...
    g_print(
        ...
        "\tLabel     : %s\n"
        "\tIcon Name : %s\n\n",
        mate_mixer_device_get_name(device), mate_mixer_device_get_label(device),
    ...
    while (switches != NULL) {
      ...
      MateMixerSwitchOption *active =
          mate_mixer_switch_get_active_option(swtch);

      g_print(
          "\tSwitch %s:\n"
          "\t\tLabel : %s\n"
          "\t\tRole  : %s\n",
          mate_mixer_switch_get_name(swtch), mate_mixer_switch_get_label(swtch),
          get_device_switch_role_string(mate_mixer_device_switch_get_role(
              MATE_MIXER_DEVICE_SWITCH(swtch))));
...
static void print_streams(void) {
  const GList *streams;
  ...
  while (streams != NULL) {
    ...
    g_print(
        ...
        "\tLabel     : %s\n"
        "\tDirection : %s\n\n",
        mate_mixer_stream_get_name(stream), mate_mixer_stream_get_label(stream),
    ...
    while (controls != NULL) {
      MateMixerStreamControl *control =
          MATE_MIXER_STREAM_CONTROL(controls->data);
      ...
      g_print(
          ...
          get_media_role_string(
              mate_mixer_stream_control_get_media_role(control)));
    ...
    while (switches != NULL) {
      ...
      g_print(
          ...
          "\t\tRole       : %s\n",
          mate_mixer_switch_get_name(swtch), mate_mixer_switch_get_label(swtch),
          get_stream_switch_role_string(mate_mixer_stream_switch_get_role(
...
static void on_context_state_notify(void) {
...
static void on_context_device_added(MateMixerContext *context,
                                    const gchar *name) {
  g_print("Device added: %s\n", name);
}

static void on_context_device_removed(MateMixerContext *context,
                                      const gchar *name) {
...
#ifdef G_OS_UNIX
static gboolean on_signal(gpointer mainloop) {
...
int main(int argc, char *argv[]) {
  ...
  GOptionEntry entries[] = {
      {"backend", 'b', 0, G_OPTION_ARG_STRING, &backend,
       "Sound system to use (pulseaudio, alsa, oss, null)", NULL},
      {"debug", 'd', 0, G_OPTION_ARG_NONE, &debug, "Enable debug", NULL},
      {"server", 's', 0, G_OPTION_ARG_STRING, &server, "Sound server address",
       NULL},
      {NULL}};
  ...
  if (debug == TRUE) g_setenv("G_MESSAGES_DEBUG", "all", FALSE);

  /* Initialize the library.
   * If the function returns FALSE, the library is not usable. */
  if (mate_mixer_init() == FALSE) return 1;
  ...
  /* Fill in some details about our application, only used with the PulseAudio
   * backend */
  mate_mixer_context_set_app_name(context, "MateMixer Monitor");
  mate_mixer_context_set_app_id(context,
                                "org.mate-desktop.libmatemixer-monitor");
  ...
  if (backend != NULL) {
    if (strcmp(backend, "pulseaudio") == 0)
      mate_mixer_context_set_backend_type(context,
      ...
      g_printerr(
          "Sound system backend '%s' is unknown, the backend will be "
          "auto-detected.\n",
          backend);
  ...
  g_signal_connect(G_OBJECT(context), "device-added",
                   G_CALLBACK(on_context_device_added), NULL);
  g_signal_connect(G_OBJECT(context), "device-removed",
                   G_CALLBACK(on_context_device_removed), NULL);
  g_signal_connect(G_OBJECT(context), "stream-added",
                   G_CALLBACK(on_context_stream_added), NULL);
  g_signal_connect(G_OBJECT(context), "stream-removed",
                   G_CALLBACK(on_context_stream_removed), NULL);
  ...
  switch (state) {
    ...
    case MATE_MIXER_STATE_CONNECTING:
      ...
      g_signal_connect(G_OBJECT(context), "notify::state",
                       G_CALLBACK(on_context_state_notify), NULL);
      break;
    default:
