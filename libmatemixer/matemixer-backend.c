...
static void mate_mixer_backend_get_property(GObject *object, guint param_id,
                                            GValue *value, GParamSpec *pspec);
static void mate_mixer_backend_set_property(GObject *object, guint param_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(MateMixerBackend, mate_mixer_backend,
                                    G_TYPE_OBJECT)
...
static void device_stream_added(MateMixerBackend *backend, const gchar *name);
static void device_stream_removed(MateMixerBackend *backend, const gchar *name);
...
static void mate_mixer_backend_class_init(MateMixerBackendClass *klass) {
  ...
  properties[PROP_DEFAULT_INPUT_STREAM] = g_param_spec_object(
      ...
      MATE_MIXER_TYPE_STREAM, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
...
static void mate_mixer_backend_set_property(GObject *object, guint param_id,
  ...
  switch (param_id) {
    case PROP_DEFAULT_INPUT_STREAM:
      mate_mixer_backend_set_default_input_stream(backend,
                                                  g_value_get_object(value));
      break;
    case PROP_DEFAULT_OUTPUT_STREAM:
      mate_mixer_backend_set_default_output_stream(backend,
                                                   g_value_get_object(value));
      break;
...
static void mate_mixer_backend_init(MateMixerBackend *backend) {
  ...
  backend->priv->devices =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);

  g_signal_connect(G_OBJECT(backend), "device-added", G_CALLBACK(device_added),
                   NULL);

  g_signal_connect(G_OBJECT(backend), "device-removed",
...
void mate_mixer_backend_set_app_info(MateMixerBackend *backend,
  ...
  if (klass->set_app_info != NULL) klass->set_app_info(backend, info);
}
...
void mate_mixer_backend_close(MateMixerBackend *backend) {
  ...
  if (klass->close != NULL) klass->close(backend);
}

MateMixerState mate_mixer_backend_get_state(MateMixerBackend *backend) {
  g_return_val_if_fail(MATE_MIXER_IS_BACKEND(backend),
...
MateMixerDevice *mate_mixer_backend_get_device(MateMixerBackend *backend,
  ...
  while (list != NULL) {
    ...
    if (strcmp(name, mate_mixer_device_get_name(device)) == 0) return device;
...
MateMixerStream *mate_mixer_backend_get_stream(MateMixerBackend *backend,
  ...
  while (list != NULL) {
    ...
    if (strcmp(name, mate_mixer_stream_get_name(stream)) == 0) return stream;
...
MateMixerStoredControl *mate_mixer_backend_get_stored_control(
    MateMixerBackend *backend, const gchar *name) {
  const GList *list;
...
const GList *mate_mixer_backend_list_devices(MateMixerBackend *backend) {
  ...
  if (klass->list_devices != NULL) return klass->list_devices(backend);
...
const GList *mate_mixer_backend_list_streams(MateMixerBackend *backend) {
  ...
  if (klass->list_streams != NULL) return klass->list_streams(backend);

  return NULL;
}

const GList *mate_mixer_backend_list_stored_controls(
    MateMixerBackend *backend) {
  MateMixerBackendClass *klass;
...
MateMixerStream *mate_mixer_backend_get_default_input_stream(
    MateMixerBackend *backend) {
  g_return_val_if_fail(MATE_MIXER_IS_BACKEND(backend), NULL);
...
gboolean mate_mixer_backend_set_default_input_stream(MateMixerBackend *backend,
  ...
  klass = MATE_MIXER_BACKEND_GET_CLASS(backend);
  if (klass->set_default_input_stream == NULL) return FALSE;
  ...
  if (backend->priv->default_input != stream) {
    ...
    if (klass->set_default_input_stream(backend, stream) == FALSE) return FALSE;
...
MateMixerStream *mate_mixer_backend_get_default_output_stream(
    MateMixerBackend *backend) {
  g_return_val_if_fail(MATE_MIXER_IS_BACKEND(backend), NULL);
...
gboolean mate_mixer_backend_set_default_output_stream(MateMixerBackend *backend,
  ...
  klass = MATE_MIXER_BACKEND_GET_CLASS(backend);
  if (klass->set_default_output_stream == NULL) return FALSE;

  if (backend->priv->default_input != stream) {
    if (mate_mixer_stream_get_direction(stream) !=
...
static void device_added(MateMixerBackend *backend, const gchar *name) {
  ...
  g_hash_table_insert(backend->priv->devices, g_strdup(name),
                      g_object_ref(device));

  /* Connect to the stream signals from devices so we can forward them on
   * the backend */
  g_signal_connect_swapped(G_OBJECT(device), "stream-added",
                           G_CALLBACK(device_stream_added), backend);
  g_signal_connect_swapped(G_OBJECT(device), "stream-removed",
                           G_CALLBACK(device_stream_removed), backend);
}
...
static void device_removed(MateMixerBackend *backend, const gchar *name) {
  ...
  g_signal_handlers_disconnect_by_func(
      G_OBJECT(device), G_CALLBACK(device_stream_added), backend);
  g_signal_handlers_disconnect_by_func(
      G_OBJECT(device), G_CALLBACK(device_stream_removed), backend);

  g_hash_table_remove(backend->priv->devices, name);
}

static void device_stream_added(MateMixerBackend *backend, const gchar *name) {
  g_signal_emit(G_OBJECT(backend), signals[STREAM_ADDED], 0, name);
}

static void device_stream_removed(MateMixerBackend *backend,
                                  const gchar *name) {
  g_signal_emit(G_OBJECT(backend), signals[STREAM_REMOVED], 0, name);
}
...
void _mate_mixer_backend_set_state(MateMixerBackend *backend,
  ...
  if (backend->priv->state == state) return;
...
void _mate_mixer_backend_set_default_input_stream(MateMixerBackend *backend,
  ...
  if (backend->priv->default_input == stream) return;
...
void _mate_mixer_backend_set_default_output_stream(MateMixerBackend *backend,
  ...
  if (backend->priv->default_output == stream) return;
