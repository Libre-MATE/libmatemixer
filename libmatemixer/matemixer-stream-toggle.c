...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(MateMixerStreamToggle,
                                    mate_mixer_stream_toggle,
                                    MATE_MIXER_TYPE_STREAM_SWITCH)

static MateMixerSwitchOption *mate_mixer_stream_toggle_get_option(
    MateMixerSwitch *swtch, const gchar *name);

static const GList *mate_mixer_stream_toggle_list_options(
    MateMixerSwitch *swtch);

static void mate_mixer_stream_toggle_class_init(
    MateMixerStreamToggleClass *klass) {
  ...
  properties[PROP_STATE] =
      g_param_spec_boolean("state", "State", "Current state of the toggle",
   ...
   */
  properties[PROP_ON_STATE_OPTION] = g_param_spec_object(
      "on-state-option", "On state option", "On state option",
   ...
   */
  properties[PROP_OFF_STATE_OPTION] = g_param_spec_object(
      "off-state-option", "Off state option", "Off state option",
      MATE_MIXER_TYPE_SWITCH_OPTION,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
...
gboolean mate_mixer_stream_toggle_get_state(MateMixerStreamToggle *toggle) {
  ...
  if (G_UNLIKELY(active == NULL)) return FALSE;
...
MateMixerSwitchOption *mate_mixer_stream_toggle_get_state_option(
    MateMixerStreamToggle *toggle, gboolean state) {
...
static MateMixerSwitchOption *mate_mixer_stream_toggle_get_option(
  ...
  if (g_strcmp0(name, mate_mixer_switch_option_get_name(toggle->priv->on)) == 0)
    return toggle->priv->on;
  if (g_strcmp0(name, mate_mixer_switch_option_get_name(toggle->priv->off)) ==
      0)
    return toggle->priv->off;
...
static const GList *mate_mixer_stream_toggle_list_options(
  ...
  if (toggle->priv->options == NULL) {
    if (G_LIKELY(toggle->priv->off != NULL))
      toggle->priv->options =
          g_list_prepend(toggle->priv->options, toggle->priv->off);
    if (G_LIKELY(toggle->priv->on != NULL))
      toggle->priv->options =
          g_list_prepend(toggle->priv->options, toggle->priv->on);
  }
