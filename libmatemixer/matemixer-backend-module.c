...
G_DEFINE_TYPE_WITH_PRIVATE(MateMixerBackendModule, mate_mixer_backend_module,
                           G_TYPE_TYPE_MODULE);

static gboolean backend_module_load(GTypeModule *gmodule);
static void backend_module_unload(GTypeModule *gmodule);

static void mate_mixer_backend_module_class_init(
    MateMixerBackendModuleClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "path", "Path", "File path to the module", NULL,
...
MateMixerBackendModule *mate_mixer_backend_module_new(const gchar *path) {
  ...
  return g_object_new(MATE_MIXER_TYPE_BACKEND_MODULE, "path", path, NULL);
}
 ...
 */
const gchar *mate_mixer_backend_module_get_path(
    MateMixerBackendModule *module) {
  g_return_val_if_fail(MATE_MIXER_IS_BACKEND_MODULE(module), NULL);
...
static gboolean backend_module_load(GTypeModule *type_module) {
  MateMixerBackendModule *module;

  module = MATE_MIXER_BACKEND_MODULE(type_module);

  if (module->priv->loaded == TRUE) return TRUE;

  module->priv->gmodule = g_module_open(
      module->priv->path, G_MODULE_BIND_LAZY | G_MODULE_BIND_LOCAL);
  if (module->priv->gmodule == NULL) {
    g_warning("Failed to load backend module %s: %s", module->priv->path,
              g_module_error());
  ...
  if (g_module_symbol(module->priv->gmodule, "backend_module_init",
                      (gpointer *)&module->priv->init) == FALSE ||
      g_module_symbol(module->priv->gmodule, "backend_module_get_info",
...
static void backend_module_unload(GTypeModule *type_module) {}
