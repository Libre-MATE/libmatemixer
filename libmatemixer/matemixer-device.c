...
enum { PROP_0, PROP_NAME, PROP_LABEL, PROP_ICON, N_PROPERTIES };

static GParamSpec *properties[N_PROPERTIES] = {
    NULL,
};

enum { STREAM_ADDED, STREAM_REMOVED, SWITCH_ADDED, SWITCH_REMOVED, N_SIGNALS };

static guint signals[N_SIGNALS] = {
    0,
};

static void mate_mixer_device_get_property(GObject *object, guint param_id,
                                           GValue *value, GParamSpec *pspec);
static void mate_mixer_device_set_property(GObject *object, guint param_id,
                                           const GValue *value,
                                           GParamSpec *pspec);

static void mate_mixer_device_finalize(GObject *object);

G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE(MateMixerDevice, mate_mixer_device,
                                    G_TYPE_OBJECT)

static MateMixerStream *mate_mixer_device_real_get_stream(
    MateMixerDevice *device, const gchar *name);
static MateMixerDeviceSwitch *mate_mixer_device_real_get_switch(
    MateMixerDevice *device, const gchar *name);
...
static void mate_mixer_device_class_init(MateMixerDeviceClass *klass) {
  ...
  properties[PROP_NAME] = g_param_spec_string(
      "name", "Name", "Name of the device", NULL,
  ...
  properties[PROP_LABEL] = g_param_spec_string(
      "label", "Label", "Label of the device", NULL,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
  ...
  properties[PROP_ICON] = g_param_spec_string(
      "icon", "Icon", "XDG icon name of the device", NULL,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
  ...
  signals[STREAM_ADDED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(MateMixerDeviceClass, stream_added), NULL, NULL,
  ...
  signals[STREAM_REMOVED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(MateMixerDeviceClass, stream_removed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  ...
  signals[SWITCH_ADDED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(MateMixerDeviceClass, switch_added), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  ...
  signals[SWITCH_REMOVED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(MateMixerDeviceClass, switch_removed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
}
...
const GList *mate_mixer_device_list_streams(MateMixerDevice *device) {
  ...
  if (G_LIKELY(klass->list_streams != NULL)) return klass->list_streams(device);
