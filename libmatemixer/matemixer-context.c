...
static void mate_mixer_context_get_property(GObject *object, guint param_id,
                                            GValue *value, GParamSpec *pspec);
static void mate_mixer_context_set_property(GObject *object, guint param_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
...
static void change_state(MateMixerContext *context, MateMixerState state);
...
static void mate_mixer_context_class_init(MateMixerContextClass *klass) {
  ...
  properties[PROP_APP_NAME] =
      g_param_spec_string("app-name", "App name", "Application name", NULL,
  ...
  properties[PROP_APP_ID] =
      g_param_spec_string("app-id", "App ID", "Application identifier", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
  ...
  properties[PROP_APP_VERSION] =
      g_param_spec_string("app-version", "App version", "Application version",
  ...
  properties[PROP_APP_ICON] =
      g_param_spec_string("app-icon", "App icon", "Application XDG icon name",
                          NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
   ...
   */
  properties[PROP_SERVER_ADDRESS] = g_param_spec_string(
      "server-address", "Server address", "Sound server address", NULL,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
  ...
  properties[PROP_STATE] =
      g_param_spec_enum("state", "State", "Current backend connection state",
                        MATE_MIXER_TYPE_STATE, MATE_MIXER_STATE_IDLE,
   ...
   */
  properties[PROP_DEFAULT_INPUT_STREAM] = g_param_spec_object(
      "default-input-stream", "Default input stream", "Default input stream",
      MATE_MIXER_TYPE_STREAM, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
   ...
   */
  properties[PROP_DEFAULT_OUTPUT_STREAM] = g_param_spec_object(
      "default-output-stream", "Default output stream", "Default output stream",
      MATE_MIXER_TYPE_STREAM, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
  ...
  signals[DEVICE_ADDED] = g_signal_new(
      "device-added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateMixerContextClass, device_added), NULL, NULL,
  ...
  signals[DEVICE_REMOVED] = g_signal_new(
      "device-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateMixerContextClass, device_removed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
   ...
   */
  signals[STREAM_ADDED] = g_signal_new(
      "stream-added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateMixerContextClass, stream_added), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  ...
  signals[STREAM_REMOVED] = g_signal_new(
      "stream-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateMixerContextClass, stream_removed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
   ...
   *
   * Use mate_mixer_context_get_stored_control() to get the
   * #MateMixerStoredControl.
   */
  signals[STORED_CONTROL_ADDED] = g_signal_new(
      "stored-control-added", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateMixerContextClass, stored_control_added), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
   ...
   * mate_mixer_context_list_stored_controls() function and it is not possible
   * to get the stored control with mate_mixer_context_get_stored_control().
   */
  signals[STORED_CONTROL_REMOVED] = g_signal_new(
      "stored-control-removed", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateMixerContextClass, stored_control_removed), NULL,
  ...
  switch (param_id) {
    case PROP_APP_NAME:
      g_value_set_string(value,
                         mate_mixer_app_info_get_name(context->priv->app_info));
      break;
    case PROP_APP_ID:
      g_value_set_string(value,
                         mate_mixer_app_info_get_id(context->priv->app_info));
      break;
    case PROP_APP_VERSION:
      g_value_set_string(
          value, mate_mixer_app_info_get_version(context->priv->app_info));
      break;
    case PROP_APP_ICON:
      g_value_set_string(value,
                         mate_mixer_app_info_get_icon(context->priv->app_info));
      break;
    ...
    case PROP_DEFAULT_INPUT_STREAM:
      g_value_set_object(value,
                         mate_mixer_context_get_default_input_stream(context));
      break;
    case PROP_DEFAULT_OUTPUT_STREAM:
      g_value_set_object(value,
                         mate_mixer_context_get_default_output_stream(context));
      break;
...
static void mate_mixer_context_set_property(GObject *object, guint param_id,
  ...
  switch (param_id) {
    ...
    case PROP_SERVER_ADDRESS:
      ...
      break;
    case PROP_DEFAULT_INPUT_STREAM:
      mate_mixer_context_set_default_input_stream(context,
                                                  g_value_get_object(value));
      break;
    case PROP_DEFAULT_OUTPUT_STREAM:
      mate_mixer_context_set_default_output_stream(context,
                                                   g_value_get_object(value));
      break;
 ...
 */
gboolean mate_mixer_context_set_backend_type(
    MateMixerContext *context, MateMixerBackendType backend_type) {
  MateMixerBackendModule *module;
  const GList *modules;
 ...
 *
 * If this function returns %TRUE, the connection has either been established,
 * or it hasn't been established yet and the result will be determined
 * asynchronously. You can differentiate between these two possibilities by
 * checking the connection #MateMixerContext:state after this function returns.
 *
 * The %MATE_MIXER_STATE_READY state indicates that the connection has been
 * established successfully.
 *
 * The %MATE_MIXER_STATE_CONNECTING state is reached when the connection has not
 * been established yet and you should wait for the state to change to either
 * %MATE_MIXER_STATE_READY or %MATE_MIXER_STATE_FAILED. It is required to have a
 * main loop running to allow an asynchronous connection to proceed. The library
 * will use the thread's default main context for this purpose.
 *
 * If this function returns %FALSE, it was not possible to connect to a sound
 * system and the #MateMixerContext:state will be set to
...
gboolean mate_mixer_context_open(MateMixerContext *context) {
  ...
  if (context->priv->backend_type != MATE_MIXER_BACKEND_UNKNOWN) {
    while (modules != NULL) {
      ...
      if (info->backend_type == context->priv->backend_type) break;
  ...
  if (info == NULL) info = mate_mixer_backend_module_get_info(module);

  context->priv->module = g_object_ref(module);
  context->priv->backend = g_object_new(info->g_type, NULL);

  mate_mixer_backend_set_app_info(context->priv->backend,
                                  context->priv->app_info);
  mate_mixer_backend_set_server_address(context->priv->backend,
                                        context->priv->server_address);

  g_debug("Trying to open backend %s", info->name);

  /* This transitional state is always present, it will change to
   * MATE_MIXER_STATE_READY or MATE_MIXER_STATE_FAILED either instantly or
   * asynchronously */
  ...
  g_signal_connect(G_OBJECT(context->priv->backend), "notify::state",
                   G_CALLBACK(on_backend_state_notify), context);
...
MateMixerState mate_mixer_context_get_state(MateMixerContext *context) {
  g_return_val_if_fail(MATE_MIXER_IS_CONTEXT(context),
...
MateMixerDevice *mate_mixer_context_get_device(MateMixerContext *context,
  ...
  if (context->priv->state != MATE_MIXER_STATE_READY) return NULL;

  return mate_mixer_backend_get_device(
      MATE_MIXER_BACKEND(context->priv->backend), name);
}
...
MateMixerStream *mate_mixer_context_get_stream(MateMixerContext *context,
  ...
  return mate_mixer_backend_get_stream(
      MATE_MIXER_BACKEND(context->priv->backend), name);
}
 ...
 *
 * Returns: a #MateMixerStoredControl or %NULL if there is no such stored
 * control.
 */
MateMixerStoredControl *mate_mixer_context_get_stored_control(
    MateMixerContext *context, const gchar *name) {
  g_return_val_if_fail(MATE_MIXER_IS_CONTEXT(context), NULL);
  g_return_val_if_fail(name != NULL, NULL);

  if (context->priv->state != MATE_MIXER_STATE_READY) return NULL;

  return mate_mixer_backend_get_stored_control(
      MATE_MIXER_BACKEND(context->priv->backend), name);
}
 ...
 * @context: a #MateMixerContext
 *
 * Gets a list of devices. Each item in the list is a #MateMixerDevice
 * representing a sound device in the system.
 *
 * The returned #GList is owned by the library and may be invalidated at any
 * time.
 *
...
const GList *mate_mixer_context_list_devices(MateMixerContext *context) {
  ...
  return mate_mixer_backend_list_devices(
      MATE_MIXER_BACKEND(context->priv->backend));
}
 ...
 * @context: a #MateMixerContext
 *
 * Gets a list of streams. Each item in the list is a #MateMixerStream
 * representing an input or output stream.
 *
...
const GList *mate_mixer_context_list_streams(MateMixerContext *context) {
  ...
  return mate_mixer_backend_list_streams(
      MATE_MIXER_BACKEND(context->priv->backend));
}
 ...
 * @context: a #MateMixerContext
 *
 * Gets a list of stored controls. Each item in the list is a
 * #MateMixerStoredControl.
 *
 * The returned #GList is owned by the library and may be invalidated at any
 * time.
 *
 * Returns: a #GList of stored controls or %NULL if there are none or you are
 * not connected to a sound system.
 */
const GList *mate_mixer_context_list_stored_controls(
  ...
  return mate_mixer_backend_list_stored_controls(
      MATE_MIXER_BACKEND(context->priv->backend));
}
...
gboolean mate_mixer_context_set_default_input_stream(MateMixerContext *context,
  ...
  if (context->priv->state != MATE_MIXER_STATE_READY) return FALSE;

  return mate_mixer_backend_set_default_input_stream(context->priv->backend,
                                                     stream);
}
 ...
 *
 * Changes the default output stream. The given @stream must be an output
 * stream.
 *
...
gboolean mate_mixer_context_set_default_output_stream(MateMixerContext *context,
  ...
  return mate_mixer_backend_set_default_output_stream(context->priv->backend,
                                                      stream);
}
 ...
 *
 * This function will not work until the @context is connected to a sound
 * system.
 *
...
const gchar *mate_mixer_context_get_backend_name(MateMixerContext *context) {
  ...
  if (context->priv->backend_chosen == FALSE) return NULL;
...
MateMixerBackendType mate_mixer_context_get_backend_type(
  ...
  if (context->priv->backend_chosen == FALSE) return MATE_MIXER_BACKEND_UNKNOWN;

  return mate_mixer_backend_module_get_info(context->priv->module)
      ->backend_type;
}
...
static void on_backend_device_added(MateMixerBackend *backend,
                                    const gchar *name,
                                    MateMixerContext *context) {
  g_signal_emit(G_OBJECT(context), signals[DEVICE_ADDED], 0, name);
}

static void on_backend_device_removed(MateMixerBackend *backend,
                                      const gchar *name,
                                      MateMixerContext *context) {
  g_signal_emit(G_OBJECT(context), signals[DEVICE_REMOVED], 0, name);
}

static void on_backend_stream_added(MateMixerBackend *backend,
                                    const gchar *name,
                                    MateMixerContext *context) {
  g_signal_emit(G_OBJECT(context), signals[STREAM_ADDED], 0, name);
}

static void on_backend_stream_removed(MateMixerBackend *backend,
                                      const gchar *name,
                                      MateMixerContext *context) {
  g_signal_emit(G_OBJECT(context), signals[STREAM_REMOVED], 0, name);
}

static void on_backend_stored_control_added(MateMixerBackend *backend,
                                            const gchar *name,
                                            MateMixerContext *context) {
  g_signal_emit(G_OBJECT(context), signals[STORED_CONTROL_ADDED], 0, name);
}

static void on_backend_stored_control_removed(MateMixerBackend *backend,
                                              const gchar *name,
                                              MateMixerContext *context) {
  g_signal_emit(G_OBJECT(context), signals[STORED_CONTROL_REMOVED], 0, name);
}
...
static void change_state(MateMixerContext *context, MateMixerState state) {
  if (context->priv->state == state) return;
  ...
  if (state == MATE_MIXER_STATE_READY &&
     ...
     * state, because the app is not allowed to query any data before that
     * state; therefore we won't end up in an inconsistent state by caching a
     * list and then missing a notification about a change in the list */
    g_signal_connect(G_OBJECT(context->priv->backend), "device-added",
                     G_CALLBACK(on_backend_device_added), context);
    g_signal_connect(G_OBJECT(context->priv->backend), "device-removed",
                     G_CALLBACK(on_backend_device_removed), context);
    g_signal_connect(G_OBJECT(context->priv->backend), "stream-added",
                     G_CALLBACK(on_backend_stream_added), context);
    g_signal_connect(G_OBJECT(context->priv->backend), "stream-removed",
                     G_CALLBACK(on_backend_stream_removed), context);
    g_signal_connect(G_OBJECT(context->priv->backend), "stored-control-added",
                     G_CALLBACK(on_backend_stored_control_added), context);
    g_signal_connect(G_OBJECT(context->priv->backend), "stored-control-removed",
                     G_CALLBACK(on_backend_stored_control_removed), context);

    g_signal_connect(
        G_OBJECT(context->priv->backend), "notify::default-input-stream",
        G_CALLBACK(on_backend_default_input_stream_notify), context);
    g_signal_connect(
        G_OBJECT(context->priv->backend), "notify::default-output-stream",
        G_CALLBACK(on_backend_default_output_stream_notify), context);
