...
#define OSS_VOLUME_JOIN_ARRAY(volume) \
  (OSS_VOLUME_JOIN((volume[LEFT_CHANNEL]), (volume[RIGHT_CHANNEL])))
...
G_DEFINE_TYPE_WITH_PRIVATE(OssStreamControl, oss_stream_control,
...
static void store_volume(OssStreamControl *control, gint volume);

static void update_balance(OssStreamControl *control);

static gboolean write_and_store_volume(OssStreamControl *control, gint volume);
...
static void oss_stream_control_finalize(GObject *object) {
  ...
  if (control->priv->fd != -1) close(control->priv->fd);

  G_OBJECT_CLASS(oss_stream_control_parent_class)->finalize(object);
}

OssStreamControl *oss_stream_control_new(const gchar *name, const gchar *label,
                                         MateMixerStreamControlRole role,
                                         OssStream *stream, gint fd,
                                         gint devnum, gboolean stereo) {
  OssStreamControl *control;
  gint newfd;
  ...
  if (stereo == TRUE) flags |= MATE_MIXER_STREAM_CONTROL_CAN_BALANCE;

  control = g_object_new(OSS_TYPE_STREAM_CONTROL, "name", name, "label", label,
                         "flags", flags, "role", role, "stream", stream, NULL);
...
static gboolean oss_stream_control_set_volume(MateMixerStreamControl *mmsc,
  ...
  if (G_UNLIKELY(control->priv->fd == -1)) return FALSE;

  return write_and_store_volume(control,
                                OSS_VOLUME_JOIN_SAME(CLAMP(volume, 0, 100)));
}
...
static guint oss_stream_control_get_channel_volume(MateMixerStreamControl *mmsc,
  ...
  if (control->priv->stereo == TRUE) {
    ...
    if (channel == LEFT_CHANNEL) return control->priv->volume[LEFT_CHANNEL];
  }
  return 0;
}
...
static MateMixerChannelPosition oss_stream_control_get_channel_position(
  ...
  if (control->priv->stereo == TRUE) {
    ...
    if (channel == LEFT_CHANNEL) return MATE_MIXER_CHANNEL_MONO;
  }
...
static guint oss_stream_control_get_normal_volume(
...
static void store_volume(OssStreamControl *control, gint volume) {
  if (control->priv->stereo == TRUE) {
    if (volume == OSS_VOLUME_JOIN_ARRAY(control->priv->volume)) return;
    ...
    if (volume == control->priv->volume[LEFT_CHANNEL]) return;
...
static gboolean write_and_store_volume(OssStreamControl *control, gint volume) {
  ...
  /* Nothing to do? */
  if (volume == OSS_VOLUME_JOIN_ARRAY(control->priv->volume)) return TRUE;
