 ...
 *
 * Also, we avoid modelling capturable channels as both input and output
 * channels, because the ones which allow capture are normally capture-only
 * channels (OSS just doesn't have the ability to make the distinction), so each
 * channel in the list contains a flag of whether it can be used as a capture
 * source, given that it's reported as capturable. Capturable channels are
 * therefore modelled as input controls and this approach avoids for example
 * putting PCM in an input stream (which makes no sense).
 *
 ...
 * true. Ideally, we should probably model a bunch of toggles, one for each
 * channel with capture capability if they are known not to be exclusive.
 */
...
typedef enum { OSS_POLL_NORMAL, OSS_POLL_RAPID } OssPollMode;

typedef enum { OSS_DEV_ANY, OSS_DEV_INPUT, OSS_DEV_OUTPUT } OssDevChannelType;
...
/* Index of a channel in the array corresponds to the channel number passed to
 * ioctl()s, device names are takes from soundcard.h */
static const OssDevChannel oss_devices[] = {
    ...
    {"treble", N_("Treble"), MATE_MIXER_STREAM_CONTROL_ROLE_TREBLE,
     OSS_DEV_OUTPUT, NULL},
    {"synth", N_("Synth"), MATE_MIXER_STREAM_CONTROL_ROLE_UNKNOWN,
     OSS_DEV_INPUT, NULL},
    {"pcm", N_("PCM"), MATE_MIXER_STREAM_CONTROL_ROLE_PCM, OSS_DEV_OUTPUT,
     NULL},
    /* OSS manual says this should be the beeper, but Linux OSS seems to assign
     * it to regular volume control */
    {"speaker", N_("Speaker"), MATE_MIXER_STREAM_CONTROL_ROLE_SPEAKER,
     OSS_DEV_OUTPUT, NULL},
    {"line", N_("Line In"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT, OSS_DEV_INPUT,
     NULL},
    {"mic", N_("Microphone"), MATE_MIXER_STREAM_CONTROL_ROLE_MICROPHONE,
     OSS_DEV_INPUT, "audio-input-microphone"},
    ...
    {"rec", N_("Record"), MATE_MIXER_STREAM_CONTROL_ROLE_MASTER, OSS_DEV_INPUT,
     NULL},
    {"igain", N_("Input Gain"), MATE_MIXER_STREAM_CONTROL_ROLE_UNKNOWN,
     OSS_DEV_INPUT, NULL},
    {"ogain", N_("Output Gain"), MATE_MIXER_STREAM_CONTROL_ROLE_UNKNOWN,
     OSS_DEV_OUTPUT, NULL},
    {"line1", N_("Line In 1"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT,
     OSS_DEV_INPUT, NULL},
    {"line2", N_("Line In 2"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT,
     OSS_DEV_INPUT, NULL},
    {"line3", N_("Line In 3"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT,
     OSS_DEV_INPUT, NULL},
    ...
    {"phin", N_("Phone In"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT, OSS_DEV_INPUT,
     NULL},
    {"phout", N_("Phone Out"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT,
     OSS_DEV_OUTPUT, NULL},
    {"video", N_("Video"), MATE_MIXER_STREAM_CONTROL_ROLE_VIDEO, OSS_DEV_INPUT,
     NULL},
    {"radio", N_("Radio"), MATE_MIXER_STREAM_CONTROL_ROLE_PORT, OSS_DEV_INPUT,
     NULL},

    /* soundcard.h on some systems include more channels, but different files
     * provide different meanings for the remaining ones and the value is
     * doubtful */
};
...
static guint create_poll_source(OssDevice *device, OssPollMode mode);
static guint create_poll_restore_source(OssDevice *device);

static void free_stream_list(OssDevice *device);

static gint compare_stream_control_devnum(gconstpointer a, gconstpointer b);
...
static void oss_device_class_init(OssDeviceClass *klass) {
  ...
  signals[CLOSED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(OssDeviceClass, closed), NULL, NULL,
...
static void oss_device_finalize(GObject *object) {
  ...
  if (device->priv->fd != -1) close(device->priv->fd);
...
OssDevice *oss_device_new(const gchar *name, const gchar *label,
                          const gchar *path, gint fd) {
  OssDevice *device;
  gint newfd;
  ...
  newfd = dup(fd);
  if (newfd == -1) {
    g_warning("Failed to duplicate file descriptor: %s", g_strerror(errno));
    return NULL;
  }

  device = g_object_new(OSS_TYPE_DEVICE, "name", name, "label", label, "icon",
                        OSS_DEVICE_ICON, NULL);
...
gboolean oss_device_open(OssDevice *device) {
  ...
  g_debug("Opening device %s (%s)", device->priv->path,
...
fail:
  g_warning("Failed to read device %s: %s", device->priv->path,
            g_strerror(errno));
...
gboolean oss_device_is_open(OssDevice *device) {
  ...
  if (device->priv->fd != -1) return TRUE;
...
void oss_device_close(OssDevice *device) {
  ...
  if (device->priv->poll_tag != 0) g_source_remove(device->priv->poll_tag);
...
void oss_device_load(OssDevice *device) {
  ...
  device->priv->input = oss_stream_new(stream_name, MATE_MIXER_DEVICE(device),
                                       MATE_MIXER_DIRECTION_INPUT);
  g_free(stream_name);

  stream_name = g_strdup_printf("oss-output-%s", name);
  device->priv->output = oss_stream_new(stream_name, MATE_MIXER_DEVICE(device),
  ...
  if (oss_stream_has_controls(device->priv->input) == TRUE) {
    controls =
        mate_mixer_stream_list_controls(MATE_MIXER_STREAM(device->priv->input));
    ...
    if (device->priv->recmask != 0) read_mixer_switch(device);
  } else
    g_clear_object(&device->priv->input);

  /* Set default output control */
  if (oss_stream_has_controls(device->priv->output) == TRUE) {
    controls = mate_mixer_stream_list_controls(
...
static const GList *oss_device_list_streams(MateMixerDevice *mmd) {
  ...
  if (device->priv->streams == NULL) {
    if (device->priv->output != NULL)
      device->priv->streams = g_list_prepend(
          device->priv->streams, g_object_ref(device->priv->output));
...
static void read_mixer_devices(OssDevice *device) {
  ...
  for (i = 0; i < OSS_N_DEVICES; i++) {
    ...
    if (OSS_MASK_HAS_DEVICE(device->priv->devmask, i) == FALSE) continue;
    ...
    control = oss_stream_control_new(
        oss_devices[i].name, gettext(oss_devices[i].label), oss_devices[i].role,
        stream, device->priv->fd, i, stereo);
    if (G_UNLIKELY(control == NULL)) continue;
    ...
    g_debug(
        "Adding device %s control %s", name,
...
static void read_mixer_switch(OssDevice *device) {
  ...
  for (i = 0; i < G_N_ELEMENTS(oss_input_priority); i++) {
    ...
    option = oss_switch_option_new(oss_devices[devnum].name,
                                   ...
                                   oss_devices[devnum].icon, devnum);
    options = g_list_prepend(options, option);
  }

  if (G_LIKELY(options != NULL))
    oss_stream_set_switch_data(device->priv->input, device->priv->fd,
...
static gboolean poll_mixer(OssDevice *device) {
  ...
  if (G_UNLIKELY(device->priv->fd == -1)) return G_SOURCE_REMOVE;
  ...
  if (load == TRUE) {
    if (device->priv->input != NULL) oss_stream_load(device->priv->input);
    if (device->priv->output != NULL) oss_stream_load(device->priv->output);
...
static guint create_poll_source(OssDevice *device, OssPollMode mode) {
  ...
  g_source_set_callback(source, (GSourceFunc)poll_mixer, device, NULL);
...
static guint create_poll_restore_source(OssDevice *device) {
  ...
  g_source_set_callback(source, (GSourceFunc)poll_mixer_restore, device, NULL);
