...
static void oss_switch_finalize(GObject *object) {
  ...
  if (swtch->priv->fd != -1) close(swtch->priv->fd);

  G_OBJECT_CLASS(oss_switch_parent_class)->finalize(object);
}

OssSwitch *oss_switch_new(OssStream *stream, const gchar *name,
                          const gchar *label, gint fd, GList *options) {
  OssSwitch *swtch;
  gint newfd;
  ...
  swtch =
      g_object_new(OSS_TYPE_SWITCH, "name", name, "label", label, "role",
                   MATE_MIXER_STREAM_SWITCH_ROLE_PORT, "stream", stream, NULL);
...
void oss_switch_load(OssSwitch *swtch) {
  ...
  if (G_UNLIKELY(swtch->priv->fd == -1)) return;
  ...
  if (recsrc != 0) {
    ...
    /* OSS shouldn't let a non-record device be selected, let's step in and
     * select something reasonable instead... */
  } else {
     ...
     * Let's not assume that's true everywhere and select something
     * explicitly... */
  }
...
static gboolean oss_switch_set_active_option(MateMixerSwitch *mms,
  ...
  if (G_UNLIKELY(swtch->priv->fd == -1)) return FALSE;
...
static OssSwitchOption *choose_default_option(OssSwitch *swtch) {
  ...
  while (list != NULL) {
    ...
    if (devnum == OSS_SWITCH_PREFERRED_DEFAULT_DEVNUM) return option;
