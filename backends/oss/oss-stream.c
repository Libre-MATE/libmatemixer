...
OssStream *oss_stream_new(const gchar *name, MateMixerDevice *device,
  ...
  return g_object_new(OSS_TYPE_STREAM, "name", name, "label", label, "device",
                      device, "direction", direction, NULL);
}

void oss_stream_add_control(OssStream *stream, OssStreamControl *control) {
  const gchar *name;
...
void oss_stream_load(OssStream *stream) {
  ...
  if (stream->priv->swtch != NULL) oss_switch_load(stream->priv->swtch);
}
...
OssStreamControl *oss_stream_get_default_control(OssStream *stream) {
  ...
  if (control != NULL) return OSS_STREAM_CONTROL(control);
...
void oss_stream_set_switch_data(OssStream *stream, gint fd, GList *options) {
  g_return_if_fail(OSS_IS_STREAM(stream));
  ...
  stream->priv->swtch = oss_switch_new(stream, OSS_STREAM_SWITCH_NAME,
                                       _("Connector"), fd, options);
  if (G_UNLIKELY(stream->priv->swtch == NULL)) return;

  /* Read the active selection */
  oss_switch_load(stream->priv->swtch);

  stream->priv->switches =
      g_list_prepend(NULL, g_object_ref(stream->priv->swtch));
  g_signal_emit_by_name(G_OBJECT(stream), "switch-added",
