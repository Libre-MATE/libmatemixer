...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(OssBackend, oss_backend, MATE_MIXER_TYPE_BACKEND,
                               0, G_ADD_PRIVATE_DYNAMIC(OssBackend))
...
static gboolean read_device(OssBackend *oss, const gchar *path,
                            gboolean *added);

static gchar *read_device_label(OssBackend *oss, const gchar *path, gint fd);

static gchar *read_device_label_sndstat(OssBackend *oss, const gchar *sndstat,
                                        const gchar *path,
                                        guint index) G_GNUC_UNUSED;

static void add_device(OssBackend *oss, OssDevice *device);
static void remove_device(OssBackend *oss, OssDevice *device);

static void remove_device_by_path(OssBackend *oss, const gchar *path);

static void remove_device_by_list_item(OssBackend *oss, GList *item);

static void remove_stream(OssBackend *oss, const gchar *name);
...
static gint compare_device_path(gconstpointer a, gconstpointer b);
...
static void oss_backend_dispose(GObject *object) {
  ...
  if (state != MATE_MIXER_STATE_IDLE) oss_backend_close(backend);
...
static gboolean oss_backend_open(MateMixerBackend *backend) {
  ...
  g_source_set_callback(oss->priv->timeout_source, (GSourceFunc)read_devices,
                        oss, NULL);
...
void oss_backend_close(MateMixerBackend *backend) {
  OssBackend *oss;
...
static gboolean read_devices(OssBackend *oss) {
  ...
  for (i = 0; i < OSS_MAX_DEVICES; i++) {
    ...
    if (added_current == TRUE) added = TRUE;
...
static gchar *read_device_label(OssBackend *oss, const gchar *path, gint fd) {
  ...
  do {
    ...
    if (ioctl(fd, SOUND_MIXER_INFO, &info) == 0) return g_strdup(info.name);
  } while (0);
#endif

  index = (guint)g_ascii_strtoull(path + sizeof("/dev/mixer") - 1, NULL, 10);
...
static gchar *read_device_label_sndstat(OssBackend *oss, const gchar *sndstat,
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    ...
    if (g_str_has_prefix(line, prefix) == FALSE) continue;
...
static void add_device(OssBackend *oss, OssDevice *device) {
  ...
  oss->priv->devices = g_list_insert_sorted_with_data(
      oss->priv->devices, device, (GCompareDataFunc)compare_devices, NULL);
  ...
  g_signal_connect_swapped(G_OBJECT(device), "closed",
                           G_CALLBACK(remove_device), oss);
  g_signal_connect_swapped(G_OBJECT(device), "stream-removed",
                           G_CALLBACK(remove_stream), oss);

  g_signal_connect_swapped(G_OBJECT(device), "closed",
                           G_CALLBACK(free_stream_list), oss);
  g_signal_connect_swapped(G_OBJECT(device), "stream-added",
                           G_CALLBACK(free_stream_list), oss);
  g_signal_connect_swapped(G_OBJECT(device), "stream-removed",
                           G_CALLBACK(free_stream_list), oss);

  g_signal_emit_by_name(G_OBJECT(oss), "device-added",
...
static void remove_device(OssBackend *oss, OssDevice *device) {
  ...
  if (item != NULL) remove_device_by_list_item(oss, item);
}
...
static void remove_device_by_list_item(OssBackend *oss, GList *item) {
  ...
  if (oss_device_is_open(device) == TRUE) oss_device_close(device);
  ...
  g_signal_emit_by_name(G_OBJECT(oss), "device-removed",
                        mate_mixer_device_get_name(MATE_MIXER_DEVICE(device)));
...
static void remove_stream(OssBackend *oss, const gchar *name) {
  ...
  stream =
      mate_mixer_backend_get_default_output_stream(MATE_MIXER_BACKEND(oss));

  if (stream != NULL && strcmp(mate_mixer_stream_get_name(stream), name) == 0)
    select_default_output_stream(oss);
}

static OssDevice *get_default_device(OssBackend *oss) {
  GList *item;

  if (oss->priv->default_device == NULL) return NULL;

  item = g_list_find_custom(oss->priv->devices, oss->priv->default_device,
                            compare_device_path);
  if (G_LIKELY(item != NULL)) return OSS_DEVICE(item->data);
...
static void free_stream_list(OssBackend *oss) {
  if (oss->priv->streams == NULL) return;
