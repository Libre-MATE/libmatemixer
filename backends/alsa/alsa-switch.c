...
G_DEFINE_TYPE_WITH_CODE(AlsaSwitch, alsa_switch, MATE_MIXER_TYPE_STREAM_SWITCH,
...
AlsaSwitch *alsa_switch_new(AlsaStream *stream, const gchar *name,
  ...
  swtch = g_object_new(ALSA_TYPE_SWITCH, "name", name, "label", label, "role",
                       role, "stream", stream, NULL);
...
static gboolean alsa_switch_set_active_option(MateMixerSwitch *mms,
  ...
  if (G_UNLIKELY(swtch->priv->element == NULL)) return FALSE;
  ...
  for (channel = 0; channel < SND_MIXER_SCHN_LAST; channel++) {
    ...
    if (swtch->priv->channel_mask & (1 << channel)) {
      gint ret =
          snd_mixer_selem_set_enum_item(swtch->priv->element, channel, index);
      if (ret == 0)
        set_item = TRUE;
      else
...
static gboolean alsa_switch_load(AlsaElement *element) {
  ...
  if (swtch->priv->channel_mask == 0) {
    for (c = 0; c < SND_MIXER_SCHN_LAST; c++) {
       ...
       * value successfully */
      if (ret == 0) swtch->priv->channel_mask |= 1 << c;
    }

    /* The last ALSA call might have failed, but it doesn't matter if we have
     * a channel mask */
    if (swtch->priv->channel_mask > 0) ret = 0;
  } else {
  ...
  g_warning("Unknown active option of switch %s: %d",
            mate_mixer_switch_get_name(MATE_MIXER_SWITCH(swtch)), item);
