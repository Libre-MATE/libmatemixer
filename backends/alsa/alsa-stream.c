...
static gint compare_control_name(gconstpointer a, gconstpointer b);
static gint compare_switch_name(gconstpointer a, gconstpointer b);
...
AlsaStream *alsa_stream_new(const gchar *name, MateMixerDevice *device,
  ...
  return g_object_new(ALSA_TYPE_STREAM, "name", name, "label", label, "device",
                      device, "direction", direction, NULL);
}

void alsa_stream_add_control(AlsaStream *stream, AlsaStreamControl *control) {
  const gchar *name;
  ...
  g_signal_emit_by_name(G_OBJECT(stream), "control-added", name);

  if (alsa_stream_has_default_control(stream) == FALSE)
    alsa_stream_set_default_control(stream, control);
}

void alsa_stream_add_switch(AlsaStream *stream, AlsaSwitch *swtch) {
  const gchar *name;
  ...
  g_signal_emit_by_name(G_OBJECT(stream), "switch-added", name);
}

void alsa_stream_add_toggle(AlsaStream *stream, AlsaToggle *toggle) {
  const gchar *name;
...
gboolean alsa_stream_has_controls(AlsaStream *stream) {
  ...
  if (stream->priv->controls != NULL) return TRUE;
...
gboolean alsa_stream_has_switches(AlsaStream *stream) {
  ...
  if (stream->priv->switches != NULL) return TRUE;
...
gboolean alsa_stream_has_controls_or_switches(AlsaStream *stream) {
  ...
  if (stream->priv->controls != NULL || stream->priv->switches != NULL)
    return TRUE;
...
AlsaStreamControl *alsa_stream_get_default_control(AlsaStream *stream) {
  ...
  if (control != NULL) return ALSA_STREAM_CONTROL(control);
...
void alsa_stream_load_elements(AlsaStream *stream, const gchar *name) {
  GList *item;
  ...
  if (item != NULL) alsa_element_load(ALSA_ELEMENT(item->data));

  item = g_list_find_custom(stream->priv->switches, name, compare_switch_name);
  if (item != NULL) alsa_element_load(ALSA_ELEMENT(item->data));
}

gboolean alsa_stream_remove_elements(AlsaStream *stream, const gchar *name) {
  GList *item;
  ...
  if (item != NULL) {
    ...
    if (control ==
        mate_mixer_stream_get_default_control(MATE_MIXER_STREAM(stream))) {
    ...
    g_signal_emit_by_name(G_OBJECT(stream), "control-removed",
  ...
  if (item != NULL) {
    ...
    g_signal_emit_by_name(G_OBJECT(stream), "switch-removed",
