...
G_DEFINE_TYPE(AlsaStreamOutputControl, alsa_stream_output_control,
...
static gboolean alsa_stream_output_control_set_volume(
    AlsaStreamControl *control, guint volume);

static gboolean alsa_stream_output_control_set_channel_volume(
    AlsaStreamControl *control, snd_mixer_selem_channel_id_t channel,
    guint volume);

static gboolean alsa_stream_output_control_get_volume_from_decibel(
    AlsaStreamControl *control, gdouble decibel, guint *volume);

static gboolean alsa_stream_output_control_get_decibel_from_volume(
    AlsaStreamControl *control, guint volume, gdouble *decibel);

static void read_volume_data(snd_mixer_elem_t *el, AlsaControlData *data);

static void alsa_stream_output_control_class_init(
    AlsaStreamOutputControlClass *klass) {
  ...
  control_class->set_channel_volume =
      alsa_stream_output_control_set_channel_volume;
  control_class->get_volume_from_decibel =
      alsa_stream_output_control_get_volume_from_decibel;
  control_class->get_decibel_from_volume =
      alsa_stream_output_control_get_decibel_from_volume;
}
...
AlsaStreamControl *alsa_stream_output_control_new(
  ...
  return g_object_new(ALSA_TYPE_STREAM_OUTPUT_CONTROL, "name", name, "label",
                      label, "role", role, "stream", stream, NULL);
}
...
static gboolean alsa_stream_output_control_load(AlsaStreamControl *control) {
  ...
  if (G_UNLIKELY(el == NULL)) return FALSE;
   ...
   * the control to have a volume, expect the control to match what we need -
   * slider with an optional mute toggle. If it fails to read the volume data,
   * just treat it as a volumeless control */
...
static gboolean alsa_stream_output_control_get_volume_from_decibel(
  ...
  ret = snd_mixer_selem_ask_playback_dB_vol(el, (glong)(decibel * 100), 0,
                                            &value);
  if (ret < 0) {
...
static void read_volume_data(snd_mixer_elem_t *el, AlsaControlData *data) {
  ...
  for (i = 0; i < MATE_MIXER_CHANNEL_MAX; i++) data->v[i] = data->min;
  ...
  if (snd_mixer_selem_is_playback_mono(el) == 1) {
    ...
    if (data->switch_usable == TRUE) {
      ...
      ret =
          snd_mixer_selem_get_playback_switch(el, SND_MIXER_SCHN_MONO, &value);
      if (G_LIKELY(ret == 0)) data->m[0] = !value;
    }
  } else {
    ...
    for (channel = 0; channel < SND_MIXER_SCHN_LAST; channel++) {
      if (snd_mixer_selem_has_playback_channel(el, channel) == 0) continue;

      if (data->switch_usable == TRUE) {
        gint value;

        ret = snd_mixer_selem_get_playback_switch(el, channel, &value);
        if (ret == 0) data->m[channel] = !value;
      }
      ...
      if (data->volume < volume) data->volume = volume;
