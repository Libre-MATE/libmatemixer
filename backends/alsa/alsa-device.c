...
#define ALSA_STREAM_CONTROL_GET_SCORE(c) \
  (GPOINTER_TO_INT(                      \
      g_object_get_data(G_OBJECT(c), "__matemixer_alsa_control_score")))

#define ALSA_STREAM_CONTROL_SET_SCORE(c, score)                     \
  (g_object_set_data(G_OBJECT(c), "__matemixer_alsa_control_score", \
                     GINT_TO_POINTER(score)))

#define ALSA_STREAM_DEFAULT_CONTROL_GET_SCORE(s) \
  (ALSA_STREAM_CONTROL_GET_SCORE(                \
      alsa_stream_get_default_control(ALSA_STREAM(s))))
...
static void add_element(AlsaDevice *device, AlsaStream *stream,
                        AlsaElement *element);

static void add_stream_input_control(AlsaDevice *device, snd_mixer_elem_t *el);
static void add_stream_output_control(AlsaDevice *device, snd_mixer_elem_t *el);

static void add_switch(AlsaDevice *device, AlsaStream *stream,
                       snd_mixer_elem_t *el);

static void add_toggle(AlsaDevice *device, AlsaStream *stream,
                       AlsaToggleType type, snd_mixer_elem_t *el);

static void add_stream_input_switch(AlsaDevice *device, snd_mixer_elem_t *el);
static void add_stream_output_switch(AlsaDevice *device, snd_mixer_elem_t *el);

static void add_stream_input_toggle(AlsaDevice *device, snd_mixer_elem_t *el);
static void add_stream_output_toggle(AlsaDevice *device, snd_mixer_elem_t *el);

static void load_element(AlsaDevice *device, snd_mixer_elem_t *el);

static void load_elements_by_name(AlsaDevice *device, const gchar *name);

static void remove_elements_by_name(AlsaDevice *device, const gchar *name);

static void handle_poll(AlsaDevice *device);

static gboolean handle_process_events(AlsaDevice *device);

static int handle_callback(snd_mixer_t *handle, guint mask,
                           snd_mixer_elem_t *el);
static int handle_element_callback(snd_mixer_elem_t *el, guint mask);
...
static void get_control_info(snd_mixer_elem_t *el, gchar **name, gchar **label,
                             MateMixerStreamControlRole *role, gint *score);
static void get_input_control_info(snd_mixer_elem_t *el, gchar **name,
                                   gchar **label,
                                   MateMixerStreamControlRole *role,
                                   gint *score);
static void get_output_control_info(snd_mixer_elem_t *el, gchar **name,
                                    gchar **label,
                                    MateMixerStreamControlRole *role,
                                    gint *score);

static MateMixerDirection get_switch_direction(snd_mixer_elem_t *el);
static void get_switch_info(snd_mixer_elem_t *el, gchar **name, gchar **label,
...
static void alsa_device_class_init(AlsaDeviceClass *klass) {
  ...
  signals[CLOSED] = g_signal_new(
      "closed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(AlsaDeviceClass, closed), NULL, NULL,
...
AlsaDevice *alsa_device_new(const gchar *name, const gchar *label) {
  AlsaDevice *device;
  gchar *stream_name;
  ...
  device = g_object_new(ALSA_TYPE_DEVICE, "name", name, "label", label, "icon",
                        ALSA_DEVICE_ICON, NULL);
  ...
  device->priv->input = alsa_stream_new(stream_name, MATE_MIXER_DEVICE(device),
                                        MATE_MIXER_DIRECTION_INPUT);
  g_free(stream_name);

  stream_name = g_strdup_printf("alsa-output-%s", name);
  device->priv->output = alsa_stream_new(stream_name, MATE_MIXER_DEVICE(device),
...
gboolean alsa_device_open(AlsaDevice *device) {
  ...
  g_debug("Opening device %s (%s)", name,
  ...
  if (ret < 0) {
    g_warning("Failed to attach mixer to %s: %s", name, snd_strerror(ret));
  ...
  if (ret < 0) {
    g_warning("Failed to register simple element for %s: %s", name,
              snd_strerror(ret));
  ...
  if (ret < 0) {
    g_warning("Failed to load mixer elements for %s: %s", name,
              snd_strerror(ret));
...
gboolean alsa_device_is_open(AlsaDevice *device) {
  ...
  if (device->priv->handle != NULL) return TRUE;
...
void alsa_device_close(AlsaDevice *device) {
  ...
  if (device->priv->handle == NULL) return;
  ...
  if (alsa_stream_has_controls_or_switches(device->priv->input) == TRUE) {
    ...
    g_signal_emit_by_name(G_OBJECT(device), "stream-removed", name);
  }
...
void alsa_device_load(AlsaDevice *device) {
  ...
  thread = g_thread_try_new("matemixer-alsa-poll", (GThreadFunc)handle_poll,
                            device, &error);
  if (thread == NULL) {
...
static void add_element(AlsaDevice *device, AlsaStream *stream,
  ...
  if (alsa_element_load(element) == FALSE) return;

  if (alsa_stream_has_controls_or_switches(stream) == FALSE) add_stream = TRUE;
  ...
  if (add_stream == TRUE) {
     ...
     * the first control */
    g_signal_emit_by_name(G_OBJECT(device), "stream-added", name);
  }
...
static void add_stream_input_control(AlsaDevice *device, snd_mixer_elem_t *el) {
  ...
  g_debug("Reading device %s input control %s",
          mate_mixer_device_get_name(MATE_MIXER_DEVICE(device)), name);

  control =
      alsa_stream_input_control_new(name, label, role, device->priv->input);
  g_free(name);
  g_free(label);
...
static void add_stream_output_control(AlsaDevice *device,
  ...
  control =
      alsa_stream_output_control_new(name, label, role, device->priv->output);
  g_free(name);
  g_free(label);
...
static void add_switch(AlsaDevice *device, AlsaStream *stream,
  ...
  if (G_UNLIKELY(count <= 0)) {
    g_debug("Skipping mixer switch %s with %d items",
            snd_mixer_selem_get_name(el), count);
    return;
  }
      ...
      for (j = 0; alsa_switch_options[j].name != NULL; j++)
        if (strcmp(item, alsa_switch_options[j].name) == 0) {
          option = alsa_switch_option_new(item,
                                          ...
                                          alsa_switch_options[j].icon, i);
          break;
        }

      if (option == NULL) option = alsa_switch_option_new(item, item, NULL, i);
  ...
  if (G_UNLIKELY(options == NULL)) return;

  get_switch_info(el, &name, &label, &role);

  /* Takes ownership of options */
  element = ALSA_ELEMENT(
      alsa_switch_new(stream, name, label, role, g_list_reverse(options)));
  g_free(name);
  g_free(label);
...
static void add_toggle(AlsaDevice *device, AlsaStream *stream,
  ...
  element =
      ALSA_ELEMENT(alsa_toggle_new(stream, name, label, role, type, on, off));
  g_free(name);
  g_free(label);
  g_object_unref(on);
  g_object_unref(off);
...
static void add_stream_input_switch(AlsaDevice *device, snd_mixer_elem_t *el) {
  g_debug("Reading device %s input switch %s (%d items)",
          ...
          snd_mixer_selem_get_name(el), snd_mixer_selem_get_enum_items(el));
...
static void load_element(AlsaDevice *device, snd_mixer_elem_t *el) {
  ...
  if (cvolume == FALSE && pvolume == FALSE) {
    ...
    if (cvolume == TRUE) add_stream_input_control(device, el);
    if (pvolume == TRUE) add_stream_output_control(device, el);
  }
}
...
static void remove_elements_by_name(AlsaDevice *device, const gchar *name) {
  if (alsa_stream_remove_elements(device->priv->input, name) == TRUE) {
    ...
    if (alsa_stream_has_controls_or_switches(device->priv->input) == FALSE) {
      ...
      free_stream_list(device);
      g_signal_emit_by_name(G_OBJECT(device), "stream-removed", stream_name);
    }
  }
...
static void handle_poll(AlsaDevice *device) {
  ...
  while (TRUE) {
    ...
    g_main_context_invoke(device->priv->context,
                          (GSourceFunc)handle_process_events, device);

    g_mutex_lock(&device->priv->mutex);

    /* Use a GCond to wait until the events are processed. The processing
     * function may be called any time later in the main loop and
     * snd_mixer_wait() returns instantly while there are pending events.
     * Without the wait, g_main_context_invoke() could be called repeatedly to
     * create idle sources until the first idle source function is called. */
    ...
    if (device->priv->handle == NULL) break;
  }
...
static gboolean handle_process_events(AlsaDevice *device) {
  ...
  if (device->priv->handle != NULL) {
    ...
    if (ret < 0) alsa_device_close(device);
  }
...
static int handle_element_callback(snd_mixer_elem_t *el, guint mask) {
  ...
  if (mask == SND_CTL_EVENT_MASK_REMOVE) {
    ...
    if (mask & SND_CTL_EVENT_MASK_VALUE) load_elements_by_name(device, name);
  }
  g_free(name);
...
static void validate_default_controls(AlsaDevice *device) {
  ...
  if (alsa_stream_has_controls(device->priv->input) == TRUE) {
    ...
    if (best_score > -1) {
      g_debug(
          "Found usable default input element %s (score %d)",
  ...
  if (alsa_stream_has_controls(device->priv->output) == TRUE) {
    ...
    if (best_score > -1) {
      g_debug(
          "Found usable default output element %s (score %d)",
          mate_mixer_stream_control_get_name(MATE_MIXER_STREAM_CONTROL(best)),
          best_score);
...
static gchar *get_element_name(snd_mixer_elem_t *el) {
  return g_strdup_printf("%s-%d", snd_mixer_selem_get_name(el),
...
static void get_control_info(snd_mixer_elem_t *el, gchar **name, gchar **label,
  ...
  for (i = 0; alsa_controls[i].name != NULL; i++) {
    if (strcmp(n, alsa_controls[i].name) != 0) continue;
...
static MateMixerDirection get_switch_direction(snd_mixer_elem_t *el) {
  ...
  if (strstr(name, "mic") != NULL || strstr(name, "adc") != NULL ||
      strstr(name, "capture") != NULL || strstr(name, "input source") != NULL)
    direction = MATE_MIXER_DIRECTION_INPUT;
  else
    direction = MATE_MIXER_DIRECTION_OUTPUT;
...
static void get_switch_info(snd_mixer_elem_t *el, gchar **name, gchar **label,
  ...
  for (i = 0; alsa_switches[i].name != NULL; i++) {
    if (strcmp(n, alsa_switches[i].name) != 0) continue;
...
static void free_stream_list(AlsaDevice *device) {
  ...
  if (device->priv->streams == NULL) return;
