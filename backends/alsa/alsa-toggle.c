...
AlsaToggle *alsa_toggle_new(AlsaStream *stream, const gchar *name,
                            const gchar *label, MateMixerStreamSwitchRole role,
                            AlsaToggleType type, AlsaSwitchOption *on,
  ...
  toggle = g_object_new(ALSA_TYPE_TOGGLE, "name", name, "label", label, "flags",
                        MATE_MIXER_STREAM_SWITCH_TOGGLE, "role", role, "stream",
                        stream, "on-state-option", on, "off-state-option", off,
                        NULL);
...
static gboolean alsa_toggle_set_active_option(MateMixerSwitch *mms,
  ...
  if (G_UNLIKELY(toggle->priv->element == NULL)) return FALSE;

  /* For toggles the 0/1 value is stored as the switch option id, there is not
   * really a need to validate that the option belong to the switch, just make
   * sure it contains the value 0 or 1 */
...
static gboolean alsa_toggle_load(AlsaElement *element) {
  ...
  if (toggle->priv->channel_mask == 0) {
    for (c = 0; c < SND_MIXER_SCHN_LAST; c++) {
      if (toggle->priv->type == ALSA_TOGGLE_CAPTURE)
        ret = snd_mixer_selem_get_capture_switch(toggle->priv->element, c,
                                                 &value);
      else
        ret = snd_mixer_selem_get_playback_switch(toggle->priv->element, c,
                                                  &value);
       ...
       * value successfully */
      if (ret == 0) toggle->priv->channel_mask |= 1 << c;
    }

    /* The last ALSA call might have failed, but it doesn't matter if we have
     * a channel mask */
    if (toggle->priv->channel_mask > 0) ret = 0;
  } else {
    ...
    if (toggle->priv->type == ALSA_TOGGLE_CAPTURE)
      ret =
          snd_mixer_selem_get_capture_switch(toggle->priv->element, c, &value);
    else
      ret =
          snd_mixer_selem_get_playback_switch(toggle->priv->element, c, &value);
  }
  ...
  if (ret == 0) {
    ...
    if (value > 0)
      active = mate_mixer_stream_toggle_get_state_option(
          MATE_MIXER_STREAM_TOGGLE(toggle), TRUE);
    else
      active = mate_mixer_stream_toggle_get_state_option(
          MATE_MIXER_STREAM_TOGGLE(toggle), FALSE);
  ...
  g_warning("Failed to read state of toggle %s: %s",
            snd_mixer_selem_get_name(toggle->priv->element), snd_strerror(ret));
