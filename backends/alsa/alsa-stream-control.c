...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE(
    AlsaStreamControl, alsa_stream_control, MATE_MIXER_TYPE_STREAM_CONTROL,
    G_ADD_PRIVATE(AlsaStreamControl)
        G_IMPLEMENT_INTERFACE(ALSA_TYPE_ELEMENT, alsa_element_interface_init))

static snd_mixer_elem_t *alsa_stream_control_get_snd_element(
    AlsaElement *element);
...
static gboolean alsa_stream_control_has_channel_position(
    MateMixerStreamControl *mmsc, MateMixerChannelPosition position);
static MateMixerChannelPosition alsa_stream_control_get_channel_position(
    MateMixerStreamControl *mmsc, guint channel);

static guint alsa_stream_control_get_channel_volume(
    MateMixerStreamControl *mmsc, guint channel);
static gboolean alsa_stream_control_set_channel_volume(
    MateMixerStreamControl *mmsc, guint channel, guint volume);

static gdouble alsa_stream_control_get_channel_decibel(
    MateMixerStreamControl *mmsc, guint channel);
static gboolean alsa_stream_control_set_channel_decibel(
...
static guint alsa_stream_control_get_normal_volume(
...
static void alsa_stream_control_class_init(AlsaStreamControlClass *klass) {
  ...
  control_class->has_channel_position =
      alsa_stream_control_has_channel_position;
  control_class->get_channel_position =
      alsa_stream_control_get_channel_position;
...
void alsa_stream_control_set_data(AlsaStreamControl *control,
  ...
  if (data->channels > 0) {
    ...
    control->priv->channel_mask =
        _mate_mixer_create_channel_mask(data->c, data->channels);
  ...
  if (flags & MATE_MIXER_STREAM_CONTROL_CAN_BALANCE)
    _mate_mixer_stream_control_set_balance(mmsc,
                                           control_data_get_balance(data));
...
static gboolean alsa_stream_control_set_mute(MateMixerStreamControl *mmsc,
  ...
  if (control->priv->data.switch_joined == TRUE) {
    if (control->priv->data.m[0] != mute) change = TRUE;
  } else {
  ...
  if (change == TRUE) {
    ...
    if (klass->set_mute(control, mute) == FALSE) return FALSE;
...
static guint alsa_stream_control_get_num_channels(
    MateMixerStreamControl *mmsc) {
...
static gboolean alsa_stream_control_set_volume(MateMixerStreamControl *mmsc,
  ...
  if (control->priv->data.volume_joined == TRUE) {
    if (control->priv->data.v[0] != volume) change = TRUE;
  } else {
  ...
  if (change == TRUE) {
    ...
    klass = ALSA_STREAM_CONTROL_GET_CLASS(control);
    if (klass->set_volume(control, volume) == FALSE) return FALSE;
...
static gboolean alsa_stream_control_has_channel_position(
  ...
  if (MATE_MIXER_CHANNEL_MASK_HAS_CHANNEL(control->priv->channel_mask,
                                          position))
    return TRUE;
  else
    return FALSE;
...
static MateMixerChannelPosition alsa_stream_control_get_channel_position(
  ...
  g_return_val_if_fail(ALSA_IS_STREAM_CONTROL(mmsc),
...
static guint alsa_stream_control_get_channel_volume(
  ...
  if (channel >= control->priv->data.channels) return 0;
...
static gboolean alsa_stream_control_set_channel_volume(
  ...
  if (channel >= control->priv->data.channels) return FALSE;
  ...
  if (volume != control->priv->data.v[channel]) {
    ...
    snd_mixer_selem_channel_id_t c =
        alsa_channel_map_to[control->priv->data.c[channel]];
    ...
    klass = ALSA_STREAM_CONTROL_GET_CLASS(control);
    if (klass->set_channel_volume(control, c, volume) == FALSE) return FALSE;
...
static gdouble alsa_stream_control_get_channel_decibel(
  ...
  if (channel >= control->priv->data.channels) return -MATE_MIXER_INFINITY;
    ...
    if (lc == TRUE || rc == TRUE) {
      ...
      if (lc == TRUE) {
        if (left == 0)
          ...
          volume = CLAMP(
              ((guint64)data->v[channel] * (guint64)nleft) / (guint64)left,
              data->min, data->max);
      } else {
        if (right == 0)
          volume = nright;
          ...
          volume = CLAMP(
              ((guint64)data->v[channel] * (guint64)nright) / (guint64)right,
              data->min, data->max);
      }

      if (klass->set_channel_volume(
              control, alsa_channel_map_to[data->c[channel]], volume) == TRUE)
    ...
    if (fc == TRUE || bc == TRUE) {
      ...
      if (fc == TRUE) {
        if (front == 0)
          ...
          volume = CLAMP(
              ((guint64)data->v[channel] * (guint64)nfront) / (guint64)front,
              data->min, data->max);
      } else {
        if (back == 0)
          volume = nback;
          ...
          volume = CLAMP(
              ((guint64)data->v[channel] * (guint64)nback) / (guint64)back,
              data->min, data->max);
      }
