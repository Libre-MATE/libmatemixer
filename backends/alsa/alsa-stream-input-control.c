...
G_DEFINE_TYPE(AlsaStreamInputControl, alsa_stream_input_control,
...
static gboolean alsa_stream_input_control_set_channel_volume(
    AlsaStreamControl *control, snd_mixer_selem_channel_id_t channel,
    guint volume);

static gboolean alsa_stream_input_control_get_volume_from_decibel(
    AlsaStreamControl *control, gdouble decibel, guint *volume);

static gboolean alsa_stream_input_control_get_decibel_from_volume(
...
static void alsa_stream_input_control_class_init(
    AlsaStreamInputControlClass *klass) {
  ...
  control_class->set_channel_volume =
      alsa_stream_input_control_set_channel_volume;
...
AlsaStreamControl *alsa_stream_input_control_new(
  ...
  return g_object_new(ALSA_TYPE_STREAM_INPUT_CONTROL, "name", name, "label",
...
static gboolean alsa_stream_input_control_get_volume_from_decibel(
  ...
  ret =
      snd_mixer_selem_ask_capture_dB_vol(el, (glong)(decibel * 100), 0, &value);
  if (ret < 0) {
...
static void read_volume_data(snd_mixer_elem_t *el, AlsaControlData *data) {
  ...
  if (snd_mixer_selem_is_capture_mono(el) == 1) {
    ...
    for (channel = 0; channel < SND_MIXER_SCHN_LAST; channel++) {
      if (snd_mixer_selem_has_capture_channel(el, channel) == 0) continue;
