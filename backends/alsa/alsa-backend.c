...
#define ALSA_DEVICE_SET_ID(d, id)                                    \
  (g_object_set_data_full(G_OBJECT(d), "__matemixer_alsa_device_id", \
                          g_strdup(id), g_free))
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(AlsaBackend, alsa_backend,
...
static gboolean read_device(AlsaBackend *alsa, const gchar *card);

static void add_device(AlsaBackend *alsa, AlsaDevice *device);

static void remove_device(AlsaBackend *alsa, AlsaDevice *device);
static void remove_device_by_name(AlsaBackend *alsa, const gchar *name);
static void remove_device_by_list_item(AlsaBackend *alsa, GList *item);

static void remove_stream(AlsaBackend *alsa, const gchar *name);
...
static gint compare_device_name(gconstpointer a, gconstpointer b);
...
static void alsa_backend_dispose(GObject *object) {
  ...
  if (state != MATE_MIXER_STATE_IDLE) alsa_backend_close(backend);
...
static gboolean udev_event_ok(struct udev_device *dev) {
  ...
  if (action && !strcmp(action, "remove")) return TRUE;
...
static gboolean udev_monitor_cb(gint fd, GIOCondition condition,
                                gpointer user_data) {
...
static gboolean udev_monitor_setup(AlsaBackend *alsa) {
  alsa->priv->udev.udev = udev_new();
  if (!alsa->priv->udev.udev) return FALSE;

  alsa->priv->udev.monitor =
      udev_monitor_new_from_netlink(alsa->priv->udev.udev, "udev");
  ...
  if (udev_monitor_filter_add_match_subsystem_devtype(alsa->priv->udev.monitor,
                                                      "sound", NULL) < 0 ||
  ...
  alsa->priv->udev.fd_source =
      g_unix_fd_add(alsa->priv->udev.fd, G_IO_IN, udev_monitor_cb, alsa);

  return TRUE;
}

static void udev_monitor_cleanup(AlsaBackend *alsa) {
  if (!alsa->priv->udev.udev) return;
...
static void timeout_source_setup(AlsaBackend *alsa) {
  ...
  g_source_set_callback(alsa->priv->timeout_source, (GSourceFunc)read_devices,
                        alsa, NULL);
  g_source_attach(alsa->priv->timeout_source,
                  g_main_context_get_thread_default());
}

static void timeout_source_cleanup(AlsaBackend *alsa) {
  if (!alsa->priv->timeout_source) return;
...
void alsa_backend_close(MateMixerBackend *backend) {
  AlsaBackend *alsa;
...
static gboolean read_devices(AlsaBackend *alsa) {
  ...
  if (read_device(alsa, "default") == TRUE) added = TRUE;

  for (num = -1;;) {
    /* Read number of the next sound card */
    ret = snd_card_next(&num);
    if (ret < 0 || num < 0) break;

    g_snprintf(card, sizeof(card), "hw:%d", num);

    if (read_device(alsa, card) == TRUE) added = TRUE;
  }
...
static void add_device(AlsaBackend *alsa, AlsaDevice *device) {
  ...
  alsa->priv->devices = g_list_insert_sorted_with_data(
      alsa->priv->devices, device, (GCompareDataFunc)compare_devices, NULL);

  /* Keep track of device identifiers */
  g_hash_table_add(alsa->priv->devices_ids,
                   g_strdup(ALSA_DEVICE_GET_ID(device)));

  g_signal_connect_swapped(G_OBJECT(device), "closed",
                           G_CALLBACK(remove_device), alsa);
  g_signal_connect_swapped(G_OBJECT(device), "stream-removed",
                           G_CALLBACK(remove_stream), alsa);

  g_signal_connect_swapped(G_OBJECT(device), "closed",
                           G_CALLBACK(free_stream_list), alsa);
  g_signal_connect_swapped(G_OBJECT(device), "stream-added",
                           G_CALLBACK(free_stream_list), alsa);
  g_signal_connect_swapped(G_OBJECT(device), "stream-removed",
                           G_CALLBACK(free_stream_list), alsa);

  g_signal_emit_by_name(G_OBJECT(alsa), "device-added",
...
static void remove_device(AlsaBackend *alsa, AlsaDevice *device) {
  ...
  if (item != NULL) remove_device_by_list_item(alsa, item);
}
...
static void remove_device_by_list_item(AlsaBackend *alsa, GList *item) {
  ...
  if (alsa_device_is_open(device) == TRUE) alsa_device_close(device);

  g_signal_handlers_disconnect_by_data(G_OBJECT(device), alsa);

  alsa->priv->devices = g_list_delete_link(alsa->priv->devices, item);

  g_hash_table_remove(alsa->priv->devices_ids, ALSA_DEVICE_GET_ID(device));

  /* The list may have been invalidated by device signals */
  free_stream_list(alsa);

  g_signal_emit_by_name(G_OBJECT(alsa), "device-removed",
                        mate_mixer_device_get_name(MATE_MIXER_DEVICE(device)));
...
static void remove_stream(AlsaBackend *alsa, const gchar *name) {
  ...
  stream =
      mate_mixer_backend_get_default_input_stream(MATE_MIXER_BACKEND(alsa));

  if (stream != NULL && strcmp(mate_mixer_stream_get_name(stream), name) == 0)
    select_default_input_stream(alsa);

  stream =
      mate_mixer_backend_get_default_output_stream(MATE_MIXER_BACKEND(alsa));
...
static void free_stream_list(AlsaBackend *alsa) {
  if (alsa->priv->streams == NULL) return;
