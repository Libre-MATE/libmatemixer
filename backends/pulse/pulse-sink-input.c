...
PulseSinkInput *pulse_sink_input_new(PulseConnection *connection,
  ...
  if (prop != NULL) {
    ...
    if (media_role == MATE_MIXER_STREAM_CONTROL_MEDIA_ROLE_EVENT) {
      ...
      if (prop != NULL) label = prop;
    }
  }

  if (label == NULL) label = info->name;

  input = g_object_new(PULSE_TYPE_SINK_INPUT, "name", name, "label", label,
                       "flags", flags, "role", role, "media-role", media_role,
                       "stream", parent, "connection", connection, "index",
                       info->index, NULL);
  g_free(name);
...
void pulse_sink_input_update(PulseSinkInput *input,
  ...
  pulse_stream_control_set_channel_map(PULSE_STREAM_CONTROL(input),
                                       &info->channel_map);
  if (info->has_volume)
    pulse_stream_control_set_cvolume(PULSE_STREAM_CONTROL(input), &info->volume,
                                     0);
  else
...
static gboolean pulse_sink_input_set_mute(PulseStreamControl *psc,
  ...
  return pulse_connection_set_sink_input_mute(
...
static gboolean pulse_sink_input_set_volume(PulseStreamControl *psc,
  ...
  return pulse_connection_set_sink_input_volume(
      pulse_stream_control_get_connection(psc),
...
static PulseMonitor *pulse_sink_input_create_monitor(PulseStreamControl *psc) {
  ...
  sink = PULSE_SINK(
      mate_mixer_stream_control_get_stream(MATE_MIXER_STREAM_CONTROL(psc)));
  ...
  return pulse_connection_create_monitor(
      pulse_stream_control_get_connection(psc), index,
