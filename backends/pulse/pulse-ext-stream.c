...
enum { PROP_0, PROP_CONNECTION, PROP_APP_INFO, N_PROPERTIES };

static GParamSpec *properties[N_PROPERTIES] = {
    NULL,
};

static void pulse_ext_stream_get_property(GObject *object, guint param_id,
                                          GValue *value, GParamSpec *pspec);
static void pulse_ext_stream_set_property(GObject *object, guint param_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
...
G_DEFINE_TYPE_WITH_PRIVATE(PulseExtStream, pulse_ext_stream,
                           MATE_MIXER_TYPE_STORED_CONTROL)

static MateMixerAppInfo *pulse_ext_stream_get_app_info(
...
static guint pulse_ext_stream_get_channel_volume(MateMixerStreamControl *mmsc,
                                                 guint channel);
static gboolean pulse_ext_stream_set_channel_volume(
    MateMixerStreamControl *mmsc, guint channel, guint volume);

static MateMixerChannelPosition pulse_ext_stream_get_channel_position(
    MateMixerStreamControl *mmsc, guint channel);
static gboolean pulse_ext_stream_has_channel_position(
...
static gboolean write_cvolume(PulseExtStream *ext, const pa_cvolume *cvolume);
static void store_cvolume(PulseExtStream *ext, const pa_cvolume *cvolume);
...
static void pulse_ext_stream_class_init(PulseExtStreamClass *klass) {
  ...
  properties[PROP_APP_INFO] = g_param_spec_boxed(
      "app-info", "Application information", "Application information",
...
PulseExtStream *pulse_ext_stream_new(PulseConnection *connection,
  ...
  if (strstr(info->name, "-by-media-role:")) {
     ...
     * available */
    if (app_info == NULL) app_info = _mate_mixer_app_info_new();
    ...
    if (G_LIKELY(suffix != NULL)) _mate_mixer_app_info_set_id(app_info, suffix);
  }

  ext = g_object_new(PULSE_TYPE_EXT_STREAM, "flags", flags, "role", role,
                     "media-role", media_role, "name", info->name, "direction",
                     direction, "stream", parent, "connection", connection,
                     "app-info", app_info, NULL);

  if (app_info != NULL) _mate_mixer_app_info_free(app_info);
...
void pulse_ext_stream_update(PulseExtStream *ext,
  ...
  if (pa_cvolume_valid(&info->volume) != 0) {
    ...
    if (ext->priv->volume != (guint)PA_VOLUME_MUTED) volume_changed = TRUE;
  }

  if (volume_changed == TRUE) store_cvolume(ext, &info->volume);
  ...
  if (parent != NULL)
    ...
    _mate_mixer_stream_control_set_stream(MATE_MIXER_STREAM_CONTROL(ext), NULL);
...
static guint pulse_ext_stream_get_channel_volume(MateMixerStreamControl *mmsc,
  ...
  if (channel >= ext->priv->cvolume.channels) return (guint)PA_VOLUME_MUTED;
...
static gboolean pulse_ext_stream_set_channel_volume(
  ...
  if (channel >= ext->priv->cvolume.channels) return FALSE;
...
static gboolean pulse_ext_stream_set_balance(MateMixerStreamControl *mmsc,
  ...
  /* Modify a temporary cvolume structure as the change may be irreversible */
  if (pa_cvolume_set_balance(&cvolume, &ext->priv->channel_map, balance) ==
      NULL)
    return FALSE;
...
static gboolean write_cvolume(PulseExtStream *ext, const pa_cvolume *cvolume) {
  ...
  if (pa_cvolume_equal(cvolume, &ext->priv->cvolume) != 0) return TRUE;
...
static void store_cvolume(PulseExtStream *ext, const pa_cvolume *cvolume) {
  ...
  value = pa_cvolume_get_balance(&ext->priv->cvolume, &ext->priv->channel_map);

  _mate_mixer_stream_control_set_balance(MATE_MIXER_STREAM_CONTROL(ext), value);

  value = pa_cvolume_get_fade(&ext->priv->cvolume, &ext->priv->channel_map);
