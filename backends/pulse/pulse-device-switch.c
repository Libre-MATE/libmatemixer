...
G_DEFINE_TYPE_WITH_PRIVATE(PulseDeviceSwitch, pulse_device_switch,
                           MATE_MIXER_TYPE_DEVICE_SWITCH)

static gboolean pulse_device_switch_set_active_option(
    MateMixerSwitch *mms, MateMixerSwitchOption *mmso);
...
static gint compare_profile_name(gconstpointer a, gconstpointer b);
...
PulseDeviceSwitch *pulse_device_switch_new(const gchar *name,
  ...
  return g_object_new(PULSE_TYPE_DEVICE_SWITCH, "name", name, "label", label,
                      "role", MATE_MIXER_DEVICE_SWITCH_ROLE_PROFILE, "device",
                      device, NULL);
}
...
void pulse_device_switch_add_profile(PulseDeviceSwitch *swtch,
  ...
  swtch->priv->profiles = g_list_insert_sorted(
      swtch->priv->profiles, g_object_ref(profile), compare_profiles);
}
...
void pulse_device_switch_set_active_profile_by_name(PulseDeviceSwitch *swtch,
  ...
  pulse_device_switch_set_active_profile(swtch,
                                         PULSE_DEVICE_PROFILE(item->data));
}
...
static gboolean pulse_device_switch_set_active_option(
  ...
  if (G_UNLIKELY(device == NULL)) return FALSE;

  device_name = mate_mixer_device_get_name(device);
  profile_name = mate_mixer_switch_option_get_name(mmso);

  return pulse_connection_set_card_profile(
      pulse_device_get_connection(PULSE_DEVICE(device)), device_name,
      profile_name);
}

static const GList *pulse_device_switch_list_options(MateMixerSwitch *swtch) {
