...
enum { PROP_0, PROP_SERVER, PROP_STATE, N_PROPERTIES };
...
static void pulse_connection_get_property(GObject *object, guint param_id,
                                          GValue *value, GParamSpec *pspec);
static void pulse_connection_set_property(GObject *object, guint param_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
...
static void pulse_subscribe_cb(pa_context *c, pa_subscription_event_type_t t,
                               uint32_t idx, void *userdata);

static void pulse_restore_subscribe_cb(pa_context *c, void *userdata);
static void pulse_server_info_cb(pa_context *c, const pa_server_info *info,
                                 void *userdata);
static void pulse_card_info_cb(pa_context *c, const pa_card_info *info, int eol,
                               void *userdata);
static void pulse_sink_info_cb(pa_context *c, const pa_sink_info *info, int eol,
                               void *userdata);
static void pulse_source_info_cb(pa_context *c, const pa_source_info *info,
...
static void pulse_connection_class_init(PulseConnectionClass *klass) {
  ...
  properties[PROP_SERVER] = g_param_spec_string(
      "server", "Server", "PulseAudio server to connect to", NULL,
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_STATE] = g_param_spec_enum(
      "state", "State", "Connection state", PULSE_TYPE_CONNECTION_STATE,
      PULSE_CONNECTION_DISCONNECTED, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

  g_object_class_install_properties(object_class, N_PROPERTIES, properties);

  signals[SERVER_INFO] = g_signal_new(
      "server-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, server_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[CARD_INFO] = g_signal_new(
      "card-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, card_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[CARD_REMOVED] = g_signal_new(
      "card-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, card_removed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[SINK_INFO] = g_signal_new(
      "sink-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, sink_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[SINK_REMOVED] = g_signal_new(
      "sink-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, sink_removed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[SINK_INPUT_INFO] = g_signal_new(
      "sink-input-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, sink_input_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[SINK_INPUT_REMOVED] = g_signal_new(
      "sink-input-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, sink_input_removed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[SOURCE_INFO] = g_signal_new(
      "source-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, source_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[SOURCE_REMOVED] = g_signal_new(
      "source-removed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, source_removed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[SOURCE_OUTPUT_INFO] = g_signal_new(
      "source-output-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, source_output_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[SOURCE_OUTPUT_REMOVED] = g_signal_new(
      "source-output-removed", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, source_output_removed), NULL, NULL,
      g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);

  signals[EXT_STREAM_LOADING] = g_signal_new(
      "ext-stream-loading", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, ext_stream_loading), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);

  signals[EXT_STREAM_LOADED] = g_signal_new(
      "ext-stream-loaded", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, ext_stream_loaded), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);

  signals[EXT_STREAM_INFO] = g_signal_new(
      "ext-stream-info", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(PulseConnectionClass, ext_stream_info), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
}
...
PulseConnection *pulse_connection_new(const gchar *app_name,
  ...
  connection =
      g_object_new(PULSE_TYPE_CONNECTION, "server", server_address, NULL);
...
gboolean pulse_connection_connect(PulseConnection *connection,
  ...
  if (connection->priv->state != PULSE_CONNECTION_DISCONNECTED) return TRUE;

  mainloop = pa_glib_mainloop_get_api(connection->priv->mainloop);
  context =
      pa_context_new_with_proplist(mainloop, NULL, connection->priv->proplist);
  ...
  pa_context_set_state_callback(context, pulse_state_cb, connection);
  if (wait_for_daemon == TRUE) flags = PA_CONTEXT_NOFAIL;

  /* Initiate a connection, state changes will be delivered asynchronously */
  if (pa_context_connect(context, connection->priv->server, flags, NULL) == 0) {
...
void pulse_connection_disconnect(PulseConnection *connection) {
  ...
  if (connection->priv->state == PULSE_CONNECTION_DISCONNECTED) return;

  if (connection->priv->context) pa_context_unref(connection->priv->context);
...
PulseConnectionState pulse_connection_get_state(PulseConnection *connection) {
  g_return_val_if_fail(PULSE_IS_CONNECTION(connection),
...
gboolean pulse_connection_load_server_info(PulseConnection *connection) {
  ...
  op = pa_context_get_server_info(connection->priv->context,
                                  pulse_server_info_cb, connection);
...
gboolean pulse_connection_load_card_info(PulseConnection *connection,
  ...
  if (index == PA_INVALID_INDEX)
    op = pa_context_get_card_info_by_index(connection->priv->context, index,
                                           pulse_card_info_cb, connection);
  else
...
gboolean pulse_connection_load_card_info_name(PulseConnection *connection,
  ...
  op = pa_context_get_card_info_by_name(connection->priv->context, name,
                                        pulse_card_info_cb, connection);
...
gboolean pulse_connection_load_sink_info(PulseConnection *connection,
  ...
  if (index == PA_INVALID_INDEX)
    op = pa_context_get_sink_info_by_index(connection->priv->context, index,
                                           pulse_sink_info_cb, connection);
  else
...
gboolean pulse_connection_load_sink_info_name(PulseConnection *connection,
  ...
  op = pa_context_get_sink_info_by_name(connection->priv->context, name,
                                        pulse_sink_info_cb, connection);
...
gboolean pulse_connection_load_sink_input_info(PulseConnection *connection,
  ...
  if (index == PA_INVALID_INDEX)
    op = pa_context_get_sink_input_info(connection->priv->context, index,
                                        pulse_sink_input_info_cb, connection);
  else
    op = pa_context_get_sink_input_info_list(
        connection->priv->context, pulse_sink_input_info_cb, connection);
...
gboolean pulse_connection_load_source_info(PulseConnection *connection,
  ...
  if (index == PA_INVALID_INDEX)
    op = pa_context_get_source_info_by_index(connection->priv->context, index,
...
gboolean pulse_connection_load_source_info_name(PulseConnection *connection,
  ...
  op = pa_context_get_source_info_by_name(connection->priv->context, name,
                                          pulse_source_info_cb, connection);
...
gboolean pulse_connection_load_source_output_info(PulseConnection *connection,
  ...
  if (index == PA_INVALID_INDEX)
    op = pa_context_get_source_output_info(connection->priv->context, index,
...
gboolean pulse_connection_load_ext_stream_info(PulseConnection *connection) {
  ...
  g_signal_emit(G_OBJECT(connection), signals[EXT_STREAM_LOADING], 0);

  op = pa_ext_stream_restore_read(connection->priv->context,
                                  pulse_ext_stream_restore_cb, connection);

  if (process_pulse_operation(connection, op) == FALSE) {
    connection->priv->ext_streams_loading = FALSE;

    g_signal_emit(G_OBJECT(connection), signals[EXT_STREAM_LOADED], 0);
    return FALSE;
  }
  return TRUE;
}
...
PulseMonitor *pulse_connection_create_monitor(PulseConnection *connection,
  ...
  if (connection->priv->state != PULSE_CONNECTION_CONNECTED) return NULL;

  return pulse_monitor_new(connection->priv->context,
                           connection->priv->proplist, index_source,
                           index_sink_input);
}
...
gboolean pulse_connection_set_default_sink(PulseConnection *connection,
  ...
  if (connection->priv->state != PULSE_CONNECTION_CONNECTED) return FALSE;

  op = pa_context_set_default_sink(connection->priv->context, name, NULL, NULL);
...
gboolean pulse_connection_set_default_source(PulseConnection *connection,
  ...
  op = pa_context_set_default_source(connection->priv->context, name, NULL,
                                     NULL);
...
gboolean pulse_connection_set_card_profile(PulseConnection *connection,
  ...
  op = pa_context_set_card_profile_by_name(connection->priv->context, card,
                                           profile, NULL, NULL);
...
gboolean pulse_connection_set_sink_mute(PulseConnection *connection,
  ...
  op = pa_context_set_sink_mute_by_index(connection->priv->context, index,
                                         (int)mute, NULL, NULL);
...
gboolean pulse_connection_set_sink_volume(PulseConnection *connection,
  ...
  op = pa_context_set_sink_volume_by_index(connection->priv->context, index,
                                           volume, NULL, NULL);
...
gboolean pulse_connection_set_sink_port(PulseConnection *connection,
                                        guint32 index, const gchar *port) {
  pa_operation *op;
  ...
  op = pa_context_set_sink_port_by_index(connection->priv->context, index, port,
                                         NULL, NULL);
...
gboolean pulse_connection_set_sink_input_mute(PulseConnection *connection,
  ...
  op = pa_context_set_sink_input_mute(connection->priv->context, index,
                                      (int)mute, NULL, NULL);
...
gboolean pulse_connection_set_sink_input_volume(PulseConnection *connection,
  ...
  op = pa_context_set_sink_input_volume(connection->priv->context, index,
                                        volume, NULL, NULL);
...
gboolean pulse_connection_set_source_mute(PulseConnection *connection,
  ...
  op = pa_context_set_source_mute_by_index(connection->priv->context, index,
                                           (int)mute, NULL, NULL);
...
gboolean pulse_connection_set_source_volume(PulseConnection *connection,
  ...
  op = pa_context_set_source_volume_by_index(connection->priv->context, index,
                                             volume, NULL, NULL);
...
gboolean pulse_connection_set_source_port(PulseConnection *connection,
  ...
  op = pa_context_set_source_port_by_index(connection->priv->context, index,
                                           port, NULL, NULL);
...
gboolean pulse_connection_set_source_output_mute(PulseConnection *connection,
  ...
  op = pa_context_set_source_output_mute(connection->priv->context, index,
                                         (int)mute, NULL, NULL);
...
gboolean pulse_connection_set_source_output_volume(PulseConnection *connection,
  ...
  op = pa_context_set_source_output_volume(connection->priv->context, index,
                                           volume, NULL, NULL);
...
gboolean pulse_connection_suspend_sink(PulseConnection *connection,
                                       guint32 index, gboolean suspend) {
  pa_operation *op;
  ...
  op = pa_context_suspend_sink_by_index(connection->priv->context, index,
                                        (int)suspend, NULL, NULL);
...
gboolean pulse_connection_suspend_source(PulseConnection *connection,
  ...
  op = pa_context_suspend_source_by_index(connection->priv->context, index,
                                          (int)suspend, NULL, NULL);
...
gboolean pulse_connection_move_sink_input(PulseConnection *connection,
                                          guint32 index, guint32 sink_index) {
  pa_operation *op;
  ...
  op = pa_context_move_sink_input_by_index(connection->priv->context, index,
...
gboolean pulse_connection_move_source_output(PulseConnection *connection,
  ...
  op = pa_context_move_source_output_by_index(connection->priv->context, index,
                                              source_index, NULL, NULL);
...
gboolean pulse_connection_kill_sink_input(PulseConnection *connection,
  ...
  op = pa_context_kill_sink_input(connection->priv->context, index, NULL, NULL);
...
gboolean pulse_connection_kill_source_output(PulseConnection *connection,
  ...
  op = pa_context_kill_source_output(connection->priv->context, index, NULL,
                                     NULL);

  return process_pulse_operation(connection, op);
}

gboolean pulse_connection_write_ext_stream(
    PulseConnection *connection, const pa_ext_stream_restore_info *info) {
  pa_operation *op;
  ...
  op = pa_ext_stream_restore_write(connection->priv->context, PA_UPDATE_REPLACE,
...
gboolean pulse_connection_delete_ext_stream(PulseConnection *connection,
  ...
  op = pa_ext_stream_restore_delete(connection->priv->context,
                                    (const char *const *)names, NULL, NULL);
...
static gchar *create_app_name(void) {
  ...
  if (name_app != NULL) return g_strdup(name_app);
...
static gboolean load_lists(PulseConnection *connection) {
  ...
  op = pa_context_get_card_info_list(connection->priv->context,
                                     pulse_card_info_cb, connection);
  if (G_UNLIKELY(op == NULL)) goto error;
...
static void pulse_state_cb(pa_context *c, void *userdata) {
  ...
  if (state == PA_CONTEXT_READY) {
    ...
    pa_context_set_subscribe_callback(connection->priv->context,
                                      pulse_subscribe_cb, connection);
    pa_ext_stream_restore_set_subscribe_cb(
        connection->priv->context, pulse_restore_subscribe_cb, connection);

    op = pa_ext_stream_restore_subscribe(connection->priv->context, TRUE, NULL,
                                         NULL);
    ...
    if (process_pulse_operation(connection, op) == TRUE) {
      ...
      if (load_lists(connection) == FALSE) state = PA_CONTEXT_FAILED;
    } else
  ...
  if (state == PA_CONTEXT_CONNECTING)
    change_state(connection, PULSE_CONNECTION_CONNECTING);
  else if (state == PA_CONTEXT_AUTHORIZING || state == PA_CONTEXT_SETTING_NAME)
  ...
  switch (t & PA_SUBSCRIPTION_EVENT_FACILITY_MASK) {
    ...
    case PA_SUBSCRIPTION_EVENT_CARD:
      if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE)
        g_signal_emit(G_OBJECT(connection), signals[CARD_REMOVED], 0, idx);
      else
    ...
    case PA_SUBSCRIPTION_EVENT_SINK:
      if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE)
        g_signal_emit(G_OBJECT(connection), signals[SINK_REMOVED], 0, idx);
      else
    ...
    case PA_SUBSCRIPTION_EVENT_SINK_INPUT:
      if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE)
        g_signal_emit(G_OBJECT(connection), signals[SINK_INPUT_REMOVED], 0,
                      idx);
      else
    ...
    case PA_SUBSCRIPTION_EVENT_SOURCE:
      if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE)
        g_signal_emit(G_OBJECT(connection), signals[SOURCE_REMOVED], 0, idx);
      else
    ...
    case PA_SUBSCRIPTION_EVENT_SOURCE_OUTPUT:
      if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE)
        g_signal_emit(G_OBJECT(connection), signals[SOURCE_OUTPUT_REMOVED], 0,
                      idx);
      else
...
static void pulse_server_info_cb(pa_context *c, const pa_server_info *info,
  ...
  g_signal_emit(G_OBJECT(connection), signals[SERVER_INFO], 0, info);
...
static void pulse_card_info_cb(pa_context *c, const pa_card_info *info, int eol,
  ...
  g_signal_emit(G_OBJECT(connection), signals[CARD_INFO], 0, info);
}
...
static void pulse_sink_info_cb(pa_context *c, const pa_sink_info *info, int eol,
  ...
  g_signal_emit(G_OBJECT(connection), signals[SINK_INFO], 0, info);
}
...
static void pulse_sink_input_info_cb(pa_context *c,
  ...
  g_signal_emit(G_OBJECT(connection), signals[SINK_INPUT_INFO], 0, info);
}
...
static void pulse_source_info_cb(pa_context *c, const pa_source_info *info,
  ...
  g_signal_emit(G_OBJECT(connection), signals[SOURCE_INFO], 0, info);
}
...
static void pulse_source_output_info_cb(pa_context *c,
  ...
  g_signal_emit(G_OBJECT(connection), signals[SOURCE_OUTPUT_INFO], 0, info);
}
...
static void pulse_ext_stream_restore_cb(pa_context *c,
  ...
  if (eol) {
    ...
    if (connection->priv->state == PULSE_CONNECTION_LOADING) {
      if (load_list_finished(connection) == FALSE) return;
    }
  ...
  g_signal_emit(G_OBJECT(connection), signals[EXT_STREAM_INFO], 0, info);
}

static void change_state(PulseConnection *connection,
                         PulseConnectionState state) {
  if (connection->priv->state == state) return;
