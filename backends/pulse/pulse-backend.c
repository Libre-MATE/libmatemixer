...
#define PULSE_SET_DEFAULT_SINK(p, s)                                    \
  (_mate_mixer_backend_set_default_output_stream(MATE_MIXER_BACKEND(p), \
                                                 MATE_MIXER_STREAM(s)))
#define PULSE_SET_DEFAULT_SOURCE(p, s)                                 \
  (_mate_mixer_backend_set_default_input_stream(MATE_MIXER_BACKEND(p), \
                                                MATE_MIXER_STREAM(s)))

#define PULSE_GET_PENDING_SINK(p) \
  (g_object_get_data(G_OBJECT(p), "__matemixer_pulse_pending_sink"))

#define PULSE_SET_PENDING_SINK(p, name)                                  \
  (g_object_set_data_full(G_OBJECT(p), "__matemixer_pulse_pending_sink", \
                          g_strdup(name), g_free))

#define PULSE_SET_PENDING_SINK_NULL(p) \
  (g_object_set_data(G_OBJECT(p), "__matemixer_pulse_pending_sink", NULL))

#define PULSE_GET_PENDING_SOURCE(p) \
  (g_object_get_data(G_OBJECT(p), "__matemixer_pulse_pending_source"))

#define PULSE_SET_PENDING_SOURCE(p, name)                                  \
  (g_object_set_data_full(G_OBJECT(p), "__matemixer_pulse_pending_source", \
                          g_strdup(name), g_free))

#define PULSE_SET_PENDING_SOURCE_NULL(p) \
  (g_object_set_data(G_OBJECT(p), "__matemixer_pulse_pending_source", NULL))

#define PULSE_GET_HANGING(o)   \
  ((gboolean)GPOINTER_TO_UINT( \
      g_object_get_data(G_OBJECT(o), "__matemixer_pulse_hanging")))

#define PULSE_SET_HANGING(o)                                   \
  (g_object_set_data(G_OBJECT(o), "__matemixer_pulse_hanging", \
                     GUINT_TO_POINTER(1)))

#define PULSE_UNSET_HANGING(o) \
  (g_object_steal_data(G_OBJECT(o), "__matemixer_pulse_hanging"))
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(PulseBackend, pulse_backend,
                               MATE_MIXER_TYPE_BACKEND, 0,
                               G_ADD_PRIVATE_DYNAMIC(PulseBackend))
...
static const GList *pulse_backend_list_stored_controls(
    MateMixerBackend *backend);

static gboolean pulse_backend_set_default_input_stream(
    MateMixerBackend *backend, MateMixerStream *stream);

static gboolean pulse_backend_set_default_output_stream(
    MateMixerBackend *backend, MateMixerStream *stream);

static void on_connection_state_notify(PulseConnection *connection,
                                       GParamSpec *pspec, PulseBackend *pulse);
...
static void on_connection_card_info(PulseConnection *connection,
                                    ...
                                    PulseBackend *pulse);
static void on_connection_card_removed(PulseConnection *connection, guint index,
                                       PulseBackend *pulse);
static void on_connection_sink_info(PulseConnection *connection,
                                    const pa_sink_info *info,
                                    PulseBackend *pulse);
static void on_connection_sink_removed(PulseConnection *connection, guint index,
                                       PulseBackend *pulse);
...
static void on_connection_sink_input_removed(PulseConnection *connection,
                                             guint index, PulseBackend *pulse);
...
static void on_connection_ext_stream_loaded(PulseConnection *connection,
                                            PulseBackend *pulse);
static void on_connection_ext_stream_info(
    PulseConnection *connection, const pa_ext_stream_restore_info *info,
    PulseBackend *pulse);

static gboolean source_try_connect(PulseBackend *pulse);

static void check_pending_sink(PulseBackend *pulse, PulseStream *stream);
static void check_pending_source(PulseBackend *pulse, PulseStream *stream);

static void remove_sink_input(PulseBackend *backend, PulseSink *sink,
                              guint index);
static void remove_source_output(PulseBackend *backend, PulseSource *source,
                                 guint index);
...
static gboolean compare_stream_names(gpointer key, gpointer value,
                                     gpointer user_data);
...
static void pulse_backend_class_init(PulseBackendClass *klass) {
  ...
  backend_class->set_default_input_stream =
      pulse_backend_set_default_input_stream;
  backend_class->set_default_output_stream =
...
static void pulse_backend_init(PulseBackend *pulse) {
  ...
  pulse->priv->devices = g_hash_table_new_full(g_direct_hash, g_direct_equal,
                                               NULL, g_object_unref);
  pulse->priv->sinks = g_hash_table_new_full(g_direct_hash, g_direct_equal,
                                             NULL, g_object_unref);
  pulse->priv->sources = g_hash_table_new_full(g_direct_hash, g_direct_equal,
                                               NULL, g_object_unref);

  pulse->priv->ext_streams =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);

  pulse->priv->sink_input_map = g_hash_table_new_full(
      g_direct_hash, g_direct_equal, NULL, g_object_unref);
  pulse->priv->source_output_map = g_hash_table_new_full(
      g_direct_hash, g_direct_equal, NULL, g_object_unref);
}
...
static void pulse_backend_dispose(GObject *object) {
  ...
  if (state != MATE_MIXER_STATE_IDLE) pulse_backend_close(backend);
...
#define PULSE_APP_VERSION(p) \
  (mate_mixer_app_info_get_version(p->priv->app_info))
...
static gboolean pulse_backend_open(MateMixerBackend *backend) {
  ...
  connection = pulse_connection_new(
      PULSE_APP_NAME(pulse), PULSE_APP_ID(pulse), PULSE_APP_VERSION(pulse),
      PULSE_APP_ICON(pulse), pulse->priv->server_address);
  ...
  g_signal_connect(G_OBJECT(connection), "notify::state",
                   G_CALLBACK(on_connection_state_notify), pulse);
  g_signal_connect(G_OBJECT(connection), "server-info",
                   G_CALLBACK(on_connection_server_info), pulse);
  g_signal_connect(G_OBJECT(connection), "card-info",
                   G_CALLBACK(on_connection_card_info), pulse);
  g_signal_connect(G_OBJECT(connection), "card-removed",
                   G_CALLBACK(on_connection_card_removed), pulse);
  g_signal_connect(G_OBJECT(connection), "sink-info",
                   G_CALLBACK(on_connection_sink_info), pulse);
  g_signal_connect(G_OBJECT(connection), "sink-removed",
                   G_CALLBACK(on_connection_sink_removed), pulse);
  g_signal_connect(G_OBJECT(connection), "sink-input-info",
                   G_CALLBACK(on_connection_sink_input_info), pulse);
  g_signal_connect(G_OBJECT(connection), "sink-input-removed",
                   G_CALLBACK(on_connection_sink_input_removed), pulse);
  g_signal_connect(G_OBJECT(connection), "source-info",
                   G_CALLBACK(on_connection_source_info), pulse);
  g_signal_connect(G_OBJECT(connection), "source-removed",
                   G_CALLBACK(on_connection_source_removed), pulse);
  g_signal_connect(G_OBJECT(connection), "source-output-info",
                   G_CALLBACK(on_connection_source_output_info), pulse);
  g_signal_connect(G_OBJECT(connection), "source-output-removed",
                   G_CALLBACK(on_connection_source_output_removed), pulse);
  g_signal_connect(G_OBJECT(connection), "ext-stream-loading",
                   G_CALLBACK(on_connection_ext_stream_loading), pulse);
  g_signal_connect(G_OBJECT(connection), "ext-stream-loaded",
                   G_CALLBACK(on_connection_ext_stream_loaded), pulse);
  g_signal_connect(G_OBJECT(connection), "ext-stream-info",
                   G_CALLBACK(on_connection_ext_stream_info), pulse);
...
static const GList *pulse_backend_list_streams(MateMixerBackend *backend) {
  ...
  if (pulse->priv->streams_list == NULL) {
    ...
    if (sinks != NULL) g_list_foreach(sinks, (GFunc)g_object_ref, NULL);

    sources = g_hash_table_get_values(pulse->priv->sources);
    if (sources != NULL) g_list_foreach(sources, (GFunc)g_object_ref, NULL);
...
static const GList *pulse_backend_list_stored_controls(
  ...
  if (pulse->priv->ext_streams_list == NULL) {
    pulse->priv->ext_streams_list =
        g_hash_table_get_values(pulse->priv->ext_streams);
    if (pulse->priv->ext_streams_list != NULL)
...
static gboolean pulse_backend_set_default_input_stream(
  ...
  if (pulse_connection_set_default_source(pulse->priv->connection, name) ==
      FALSE)
    return FALSE;

  /* We might be in the process of setting a default source for which the
   * details are not yet known, make sure the change does not happen */
  ...
  switch (state) {
    case PULSE_CONNECTION_DISCONNECTED:
      if (pulse->priv->connected_once == TRUE) {
        ...
        if (G_UNLIKELY(pulse->priv->connect_tag != 0)) break;

        if (pulse_connection_connect(connection, TRUE) == FALSE) {
          GSource *source;

          source = g_timeout_source_new(200);
          g_source_set_callback(source, (GSourceFunc)source_try_connect, pulse,
                                NULL);
...
static void on_connection_server_info(PulseConnection *connection,
  ...
  if (stream != NULL) name_source = mate_mixer_stream_get_name(stream);

  if (g_strcmp0(name_source, info->default_source_name) != 0) {
    if (info->default_source_name != NULL) {
      MateMixerStream *stream =
          g_hash_table_find(pulse->priv->sources, compare_stream_names,
  ...
  if (stream != NULL) name_sink = mate_mixer_stream_get_name(stream);

  if (g_strcmp0(name_sink, info->default_sink_name) != 0) {
    if (info->default_sink_name != NULL) {
      MateMixerStream *stream =
          g_hash_table_find(pulse->priv->sinks, compare_stream_names,
  ...
  if (mate_mixer_backend_get_state(MATE_MIXER_BACKEND(pulse)) !=
      MATE_MIXER_STATE_READY)
    g_debug("Sound server is %s version %s, running on %s", info->server_name,
            info->server_version, info->host_name);
}
...
static void on_connection_card_info(PulseConnection *connection,
  ...
  device =
      g_hash_table_lookup(pulse->priv->devices, GUINT_TO_POINTER(info->index));
  if (device == NULL) {
    device = pulse_device_new(connection, info);

    g_hash_table_insert(pulse->priv->devices, GUINT_TO_POINTER(info->index),
                        device);

    free_list_devices(pulse);
    g_signal_emit_by_name(
        G_OBJECT(pulse), "device-added",
...
static void on_connection_card_removed(PulseConnection *connection, guint index,
  ...
  if (G_UNLIKELY(device == NULL)) return;
...
static void on_connection_sink_info(PulseConnection *connection,
  ...
  if (info->card != PA_INVALID_INDEX)
    device =
        g_hash_table_lookup(pulse->priv->devices, GUINT_TO_POINTER(info->card));

  stream =
      g_hash_table_lookup(pulse->priv->sinks, GUINT_TO_POINTER(info->index));
  if (stream == NULL) {
    stream = PULSE_STREAM(pulse_sink_new(connection, info, device));

    g_hash_table_insert(pulse->priv->sinks, GUINT_TO_POINTER(info->index),
                        stream);
    ...
    if (device != NULL) {
      ...
      const gchar *name = mate_mixer_stream_get_name(MATE_MIXER_STREAM(stream));

      /* Only emit when not a part of the device, otherwise emitted by
       * the main library */
      g_signal_emit_by_name(G_OBJECT(pulse), "stream-added", name);
    }
...
static void on_connection_sink_removed(PulseConnection *connection, guint idx,
                                       PulseBackend *pulse) {
  PulseStream *stream;
  PulseDevice *device;

  stream = g_hash_table_lookup(pulse->priv->sinks, GUINT_TO_POINTER(idx));
  if (G_UNLIKELY(stream == NULL)) return;
...
static void on_connection_sink_input_info(PulseConnection *connection,
  ...
  if (G_LIKELY(info->sink != PA_INVALID_INDEX))
    sink =
        g_hash_table_lookup(pulse->priv->sinks, GUINT_TO_POINTER(info->sink));

  if (G_UNLIKELY(sink == NULL)) {
    prev = g_hash_table_lookup(pulse->priv->sink_input_map,
                               GUINT_TO_POINTER(info->index));
    if (prev != NULL) {
      g_debug(
          "Sink input %u moved from sink %s to an unknown sink %u, removing",
          info->index, mate_mixer_stream_get_name(MATE_MIXER_STREAM(prev)),
          info->sink);
  ...
  if (pulse_sink_add_input(sink, info) == TRUE)
    g_hash_table_insert(pulse->priv->sink_input_map,
                        GUINT_TO_POINTER(info->index), g_object_ref(sink));
}

static void on_connection_sink_input_removed(PulseConnection *connection,
                                             guint idx, PulseBackend *pulse) {
  PulseSink *sink;

  sink =
      g_hash_table_lookup(pulse->priv->sink_input_map, GUINT_TO_POINTER(idx));
  if (G_UNLIKELY(sink == NULL)) return;
...
static void on_connection_source_info(PulseConnection *connection,
  ...
  stream =
      g_hash_table_lookup(pulse->priv->sources, GUINT_TO_POINTER(info->index));
  if (stream == NULL) {
    stream = PULSE_STREAM(pulse_source_new(connection, info, device));

    g_hash_table_insert(pulse->priv->sources, GUINT_TO_POINTER(info->index),
                        stream);
...
static void on_connection_source_removed(PulseConnection *connection, guint idx,
                                         PulseBackend *pulse) {
  PulseDevice *device;
  PulseStream *stream;
...
static void on_connection_source_output_info(PulseConnection *connection,
  ...
  if (G_LIKELY(info->source != PA_INVALID_INDEX))
    source = g_hash_table_lookup(pulse->priv->sources,
                                 GUINT_TO_POINTER(info->source));

  if (G_UNLIKELY(source == NULL)) {
    prev = g_hash_table_lookup(pulse->priv->source_output_map,
                               GUINT_TO_POINTER(info->index));
    if (prev != NULL) {
      g_debug(
          "Source output %u moved from source %s to an unknown source %u, "
          "removing",
          info->index, mate_mixer_stream_get_name(MATE_MIXER_STREAM(prev)),
          info->source);
  ...
  if (pulse_source_add_output(source, info) == TRUE)
    g_hash_table_insert(pulse->priv->source_output_map,
                        GUINT_TO_POINTER(info->index), g_object_ref(source));
}
...
static void on_connection_source_output_removed(PulseConnection *connection,
  ...
  source = g_hash_table_lookup(pulse->priv->source_output_map,
                               GUINT_TO_POINTER(idx));
  if (G_UNLIKELY(source == NULL)) return;
...
static void on_connection_ext_stream_info(
  ...
  if (ext == NULL) {
    ...
    g_hash_table_insert(pulse->priv->ext_streams, g_strdup(info->name), ext);

    free_list_ext_streams(pulse);

    g_signal_emit_by_name(
        G_OBJECT(pulse), "stored-control-added",
...
static void on_connection_ext_stream_loaded(PulseConnection *connection,
  ...
  while (g_hash_table_iter_next(&iter, &name, &ext) == TRUE) {
    if (PULSE_GET_HANGING(ext) == FALSE) continue;

    g_hash_table_iter_remove(&iter);
    free_list_ext_streams(pulse);

    g_signal_emit_by_name(G_OBJECT(pulse), "stored-control-removed", name);
  }
}
...
static void check_pending_sink(PulseBackend *pulse, PulseStream *stream) {
  ...
  if (g_strcmp0(pending, name) != 0) return;
...
static void remove_sink_input(PulseBackend *pulse, PulseSink *sink,
                              guint index) {
...
static void remove_source_output(PulseBackend *pulse, PulseSource *source,
                                 guint index) {
...
static void free_list_devices(PulseBackend *pulse) {
  if (pulse->priv->devices_list == NULL) return;
...
static void free_list_streams(PulseBackend *pulse) {
  if (pulse->priv->streams_list == NULL) return;
...
static void free_list_ext_streams(PulseBackend *pulse) {
  if (pulse->priv->ext_streams_list == NULL) return;
