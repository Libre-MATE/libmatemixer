...
static void pulse_device_get_property(GObject *object, guint param_id,
                                      GValue *value, GParamSpec *pspec);
static void pulse_device_set_property(GObject *object, guint param_id,
...
static void pulse_device_load(PulseDevice *device, const pa_card_info *info);
...
static void pulse_device_class_init(PulseDeviceClass *klass) {
  ...
  properties[PROP_INDEX] = g_param_spec_uint(
      "index", "Index", "Index of the device", 0, G_MAXUINT, 0,
...
PulseDevice *pulse_device_new(PulseConnection *connection,
  ...
  if (G_UNLIKELY(label == NULL)) label = info->name;

  icon = pa_proplist_gets(info->proplist, PA_PROP_DEVICE_ICON_NAME);
  if (G_UNLIKELY(icon == NULL)) icon = "audio-card";

  /* Consider the device index as unchanging parameter */
  device = g_object_new(PULSE_TYPE_DEVICE, "index", info->index, "connection",
                        connection, "name", info->name, "label", label, "icon",
                        icon, NULL);
...
void pulse_device_update(PulseDevice *device, const pa_card_info *info) {
...
void pulse_device_add_stream(PulseDevice *device, PulseStream *stream) {
  const gchar *name;
  ...
  g_hash_table_insert(device->priv->streams, g_strdup(name),
                      g_object_ref(stream));
...
void pulse_device_remove_stream(PulseDevice *device, PulseStream *stream) {
  const gchar *name;
...
PulsePort *pulse_device_get_port(PulseDevice *device, const gchar *name) {
  g_return_val_if_fail(PULSE_IS_DEVICE(device), NULL);
...
static void pulse_device_load(PulseDevice *device, const pa_card_info *info) {
  ...
  for (i = 0; i < info->n_ports; i++) {
    ...
    port = pulse_port_new(name, info->ports[i]->description, icon,
  ...
  if (info->n_profiles > 0) {
    device->priv->pswitch =
        pulse_device_switch_new("profile", _("Profile"), device);
  ...
  for (i = 0; i < info->n_profiles; i++) {
    ...
    if (p_info->available == 0) continue;

    profile = pulse_device_profile_new(p_info->name, p_info->description,
                                       p_info->priority);
...
static void free_list_streams(PulseDevice *device) {
  if (device->priv->streams_list == NULL) return;
